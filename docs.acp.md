...
This document describes how the Kolb-Bot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`kolb-bot acp` exposes an ACP agent over stdio and forwards prompts to a running
Kolb-Bot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Kolb-Bot Gateway
...
- ACP client spawns `kolb-bot acp` and speaks ACP messages over stdio.
...
`kolb-bot acp` resolves the Gateway URL and auth from CLI flags or config:
