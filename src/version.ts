...
declare const __KOLB_BOT_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "kolb-bot";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["KOLB_BOT_VERSION"],
      runtimeVersion,
      env["KOLB_BOT_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Kolb-Bot version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __KOLB_BOT_VERSION__ === "string" ? __KOLB_BOT_VERSION__ : undefined,
  bundledVersion: process.env.KOLB_BOT_BUNDLED_VERSION,
});
