...
const DEFAULT_PACKAGE_NAME = "kolb-bot";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["kolb-bot", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "kolb-bot.mjs");
    ...
    const doctorStep = await runStep(
      step("kolb-bot doctor", doctorArgv, gitRoot, { KOLB_BOT_UPDATE_IN_PROGRESS: "1" }),
    );
