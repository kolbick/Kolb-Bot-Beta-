...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.KOLB_BOT_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Kolb-Bot. You can keep running the web gateway without it: \`${formatCliCommand("kolb-bot gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
