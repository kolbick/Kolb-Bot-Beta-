...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["KOLB_BOT_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.KOLB_BOT_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveKolbBotPackageRoot } = await import("./kolb-bot-root.js"));
    ...
    vi.mocked(resolveKolbBotPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveKolbBotPackageRoot).mockResolvedValue("/opt/kolb-bot");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/kolb-bot/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
