...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.KOLB_BOT_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.KOLB_BOT_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set KOLB_BOT_APNS_TEAM_ID and KOLB_BOT_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.KOLB_BOT_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.KOLB_BOT_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.KOLB_BOT_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set KOLB_BOT_APNS_PRIVATE_KEY_P8 or KOLB_BOT_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading KOLB_BOT_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createKolbBotPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    "kolb-bot": createKolbBotPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
