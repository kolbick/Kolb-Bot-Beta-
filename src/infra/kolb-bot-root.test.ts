...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__kolb-bot_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "kolb-bot-root");
...
describe("resolveKolbBotPackageRoot", () => {
  let resolveKolbBotPackageRoot: typeof import("./kolb-bot-root.js").resolveKolbBotPackageRoot;
  let resolveKolbBotPackageRootSync: typeof import("./kolb-bot-root.js").resolveKolbBotPackageRootSync;

  beforeAll(async () => {
    ({ resolveKolbBotPackageRoot, resolveKolbBotPackageRootSync } =
      await import("./kolb-bot-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "kolb-bot");
    const pkgRoot = path.join(project, "node_modules", "kolb-bot");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "kolb-bot" }));

    expect(resolveKolbBotPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "kolb-bot");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "kolb-bot.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "kolb-bot" }));

    expect(resolveKolbBotPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveKolbBotPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveKolbBotPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveKolbBotPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-kolb-bot package roots", async () => {
    const pkgRoot = fx("not-kolb-bot");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-kolb-bot" }));

    expect(resolveKolbBotPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveKolbBotPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveKolbBotPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
