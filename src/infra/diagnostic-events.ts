...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __kolbBotDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__kolbBotDiagnosticEventsState) {
    globalStore.__kolbBotDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__kolbBotDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: KolbBotConfig): boolean {
