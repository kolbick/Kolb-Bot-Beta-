const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "KOLB_BOT_LAUNCHD_LABEL"],
  systemd: ["KOLB_BOT_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["KOLB_BOT_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "KOLB_BOT_SERVICE_MARKER",
  "KOLB_BOT_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.KOLB_BOT_SERVICE_MARKER?.trim();
    const serviceKind = env.KOLB_BOT_SERVICE_KIND?.trim();
