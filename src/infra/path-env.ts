...
function candidateBinDirs(opts: EnsureKolbBotPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `kolb-bot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "kolb-bot");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.KOLB_BOT_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "kolb-bot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `kolb-bot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureKolbBotCliOnPath(opts: EnsureKolbBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.KOLB_BOT_PATH_BOOTSTRAPPED)) {
    return;
  }
