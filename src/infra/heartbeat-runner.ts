...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: KolbBotConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: KolbBotConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: KolbBotConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: KolbBotConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: KolbBotConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: KolbBotConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: KolbBotConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: KolbBotConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
