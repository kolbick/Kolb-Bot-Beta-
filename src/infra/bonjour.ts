...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.KOLB_BOT_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Kolb-Bot";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Kolb-Bot\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.KOLB_BOT_MDNS_HOSTNAME?.trim() ||
    process.env.KOLB_BOT_MDNS_HOSTNAME?.trim() ||
    "kolb-bot";
  const hostname =
    hostnameRaw
