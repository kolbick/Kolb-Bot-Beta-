...
export function triggerKolbBotRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.KOLB_BOT_SYSTEMD_UNIT,
      process.env.KOLB_BOT_PROFILE,
    );
  ...
  const label =
    process.env.KOLB_BOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.KOLB_BOT_PROFILE);
