...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: KolbBotConfig | null = null;
  ...
  get(key: string, cfg: KolbBotConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: KolbBotConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: KolbBotConfig): void {
