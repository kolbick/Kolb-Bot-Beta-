...
let ensureKolbBotCliOnPath: typeof import("./path-env.js").ensureKolbBotCliOnPath;

describe("ensureKolbBotCliOnPath", () => {
  const envKeys = [
    "PATH",
    "KOLB_BOT_PATH_BOOTSTRAPPED",
    "KOLB_BOT_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureKolbBotCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling kolb-bot exists", () => {
    const tmp = abs("/tmp/kolb-bot-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "kolb-bot");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.KOLB_BOT_PATH_BOOTSTRAPPED;

    ensureKolbBotCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.KOLB_BOT_PATH_BOOTSTRAPPED = "1";
    ensureKolbBotCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/kolb-bot-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "kolb-bot");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/kolb-bot-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "kolb-bot");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "kolb-bot");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/kolb-bot-path/case-linuxbrew");
