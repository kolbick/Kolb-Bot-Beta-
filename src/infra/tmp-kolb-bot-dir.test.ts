...
import { POSIX_KOLB_BOT_TMP_DIR, resolvePreferredKolbBotTmpDir } from "./tmp-kolb-bot-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredKolbBotTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `kolb-bot-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredKolbBotTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_KOLB_BOT_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredKolbBotTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredKolbBotTmpDir", () => {
  it("prefers /tmp/kolb-bot when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_KOLB_BOT_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/kolb-bot when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_KOLB_BOT_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/kolb-bot when /tmp/kolb-bot is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/kolb-bot when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/kolb-bot is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/kolb-bot is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/kolb-bot is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Kolb-Bot temp dir/);
  });
