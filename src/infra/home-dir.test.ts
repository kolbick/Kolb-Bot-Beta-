...
describe("resolveEffectiveHomeDir", () => {
  it("prefers KOLB_BOT_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/kolb-bot-home"),
    );
  });
  ...
  it("expands KOLB_BOT_HOME when set to ~", () => {
    const env = {
      KOLB_BOT_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for KOLB_BOT_HOME", () => {
    const result = resolveRequiredHomeDir(
      { KOLB_BOT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when KOLB_BOT_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ KOLB_BOT_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { KOLB_BOT_HOME: "/srv/kolb-bot-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/kolb-bot-home")}/x`);
  });
