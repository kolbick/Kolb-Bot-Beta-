...
import { triggerKolbBotRestart } from "./restart.js";
 ...
 * - KOLB_BOT_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.KOLB_BOT_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    // See: https://github.com/kolbick/Kolb-Bot-Beta-/issues/39760
    if (supervisor === "schtasks") {
      const restart = triggerKolbBotRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
