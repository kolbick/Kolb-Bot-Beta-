 ...
 * returning control to the caller (typically `triggerKolbBotRestart` →
...
/**
 * Parse kolb-bot gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("kolb-bot")) {
 ...
 * Returns only PIDs that belong to kolb-bot gateway processes (not the current process).
 */
