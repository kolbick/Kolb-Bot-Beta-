...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: KolbBotConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: KolbBotConfig) => { changed: boolean; next: KolbBotConfig };
  defaultModel: string;
  overrideConfig: KolbBotConfig;
  alreadyDefaultConfig: KolbBotConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: KolbBotConfig) => config,
      applyDefaultConfig: (config: KolbBotConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: KolbBotConfig = { agents: { defaults: {} } };
