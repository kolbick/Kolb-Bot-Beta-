...
export async function loadValidConfigOrThrow(): Promise<KolbBotConfig> {
...
export async function updateConfig(
  mutator: (cfg: KolbBotConfig) => KolbBotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: KolbBotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: KolbBotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("kolb-bot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
