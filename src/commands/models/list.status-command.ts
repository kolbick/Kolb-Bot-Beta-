...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveKolbBotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("kolb-bot models auth setup-token")}\` or \`${formatCliCommand("kolb-bot configure")}\`.`
          : `Run \`${formatCliCommand("kolb-bot configure")}\` or set an API key env var.`;
