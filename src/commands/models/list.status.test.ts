...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveKolbBotAgentDir: vi.fn().mockReturnValue("/tmp/kolb-bot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/kolb-bot-agent"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/kolb-bot-dev/kolb-bot.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveKolbBotAgentDir: mocks.resolveKolbBotAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/kolb-bot-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveKolbBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/kolb-bot-dev/kolb-bot.json");
    expect(payload.auth.storePath).toBe("/tmp/kolb-bot-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/kolb-bot-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/kolb-bot-agent-custom");
