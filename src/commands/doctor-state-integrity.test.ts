...
type EnvSnapshot = {
  HOME?: string;
  KOLB_BOT_HOME?: string;
  KOLB_BOT_STATE_DIR?: string;
  KOLB_BOT_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    KOLB_BOT_HOME: process.env.KOLB_BOT_HOME,
    KOLB_BOT_STATE_DIR: process.env.KOLB_BOT_STATE_DIR,
    KOLB_BOT_OAUTH_DIR: process.env.KOLB_BOT_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: KolbBotConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: KolbBotConfig) {
...
async function runStateIntegrityText(cfg: KolbBotConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.KOLB_BOT_HOME = tempHome;
    process.env.KOLB_BOT_STATE_DIR = path.join(tempHome, ".kolb-bot");
    delete process.env.KOLB_BOT_OAUTH_DIR;
    fs.mkdirSync(process.env.KOLB_BOT_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when KOLB_BOT_OAUTH_DIR is explicitly configured", async () => {
    process.env.KOLB_BOT_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: KolbBotConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints kolb-bot-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: KolbBotConfig = {};
    ...
    expect(text).toMatch(/kolb-bot sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/kolb-bot sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /kolb-bot sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
