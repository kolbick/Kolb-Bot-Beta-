...
type PromptDefaultModelResult = { model?: string; config?: KolbBotConfig };
...
function resolveConfiguredModelRaw(cfg: KolbBotConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: KolbBotConfig): string[] {
...
export function applyPrimaryModel(cfg: KolbBotConfig, model: string): KolbBotConfig {
...
export function applyModelAllowlist(cfg: KolbBotConfig, models: string[]): KolbBotConfig {
  const defaults = cfg.agents?.defaults;
