...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.KOLB_BOT_GATEWAY_TOKEN ?? process.env.KOLB_BOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from kolb-bot.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Kolb-Bot update wizard" : "Kolb-Bot configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: KolbBotConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("kolb-bot doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
