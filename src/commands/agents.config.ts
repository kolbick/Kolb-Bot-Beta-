...
type AgentEntry = NonNullable<NonNullable<KolbBotConfig["agents"]>["list"]>[number];
...
function resolveAgentName(cfg: KolbBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: KolbBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: KolbBotConfig): AgentSummary[] {
