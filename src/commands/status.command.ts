...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByKolbBot ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByKolbBot ? "installed · " : "";
  ...
  runtime.log(theme.heading("Kolb-Bot status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`kolb-bot devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("kolb-bot devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("kolb-bot devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("kolb-bot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("kolb-bot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.github.com/kolbick/Kolb-Bot-Beta-/faq");
  runtime.log("Troubleshooting: https://docs.github.com/kolbick/Kolb-Bot-Beta-/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("kolb-bot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("kolb-bot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("kolb-bot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("kolb-bot gateway probe")}`);
  }
}
