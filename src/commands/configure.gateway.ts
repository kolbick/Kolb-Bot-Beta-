...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "KOLB_BOT_GATEWAY_TOKEN",
          placeholder: "KOLB_BOT_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like KOLB_BOT_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Kolb-Bot will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Kolb-Bot trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.github.com/kolbick/Kolb-Bot-Beta-/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
