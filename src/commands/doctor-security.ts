...
import type { KolbBotConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: KolbBotConfig): string[] {
...
export async function noteSecurityWarnings(cfg: KolbBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("kolb-bot security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.kolb-bot/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("kolb-bot approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.github.com/kolbick/Kolb-Bot-Beta-/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("kolb-bot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("kolb-bot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("kolb-bot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("kolb-bot config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('kolb-bot config set session.dmScope "per-channel-peer"') +
