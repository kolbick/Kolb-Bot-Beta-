...
import { resolveKolbBotPackageRoot } from "../infra/kolb-bot-root.js";
...
function resolveMode(cfg: KolbBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Kolb-Bot doctor");

  const root = await resolveKolbBotPackageRoot({
  ...
  let cfg: KolbBotConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("kolb-bot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("kolb-bot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("kolb-bot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("kolb-bot config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("kolb-bot config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("kolb-bot doctor --fix")}" to apply changes.`);
  }
