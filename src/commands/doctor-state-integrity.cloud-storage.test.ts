...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Kolb-Bot", ".kolb-bot");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/kolb-bot-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".kolb-bot");
  ...
  it("anchors cloud detection to OS homedir when KOLB_BOT_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".kolb-bot");
    const originalKolbBotHome = process.env.KOLB_BOT_HOME;
    process.env.KOLB_BOT_HOME = "/tmp/kolb-bot-home-override";
    ...
    try {
      ...
      if (originalKolbBotHome === undefined) {
        delete process.env.KOLB_BOT_HOME;
      } else {
        process.env.KOLB_BOT_HOME = originalKolbBotHome;
      }
    }
  });
