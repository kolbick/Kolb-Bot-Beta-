...
beforeAll(() => {
  envSnapshot = captureEnv(["KOLB_BOT_PROFILE"]);
  process.env.KOLB_BOT_PROFILE = "isolated";
});
...
vi.mock("../infra/kolb-bot-root.js", () => ({
  resolveKolbBotPackageRoot: vi.fn().mockResolvedValue("/tmp/kolb-bot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/kolb-bot/pnpm-lock.yaml",
      markerPath: "/tmp/kolb-bot/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.kolb-bot.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.kolb-bot.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("kolb-bot status --all") ||
          line.includes("kolb-bot --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("KOLB_BOT_GATEWAY_TOKEN", "abcd1234", async () => {
