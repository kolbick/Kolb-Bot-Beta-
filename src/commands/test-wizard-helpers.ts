...
export function requireKolbBotAgentDir(): string {
  const agentDir = process.env.KOLB_BOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("KOLB_BOT_AGENT_DIR not set");
  }
  return agentDir;
}
