...
export function normalizeCompatibilityConfigValues(cfg: KolbBotConfig): {
  config: KolbBotConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: KolbBotConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as KolbBotConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as KolbBotConfig["browser"],
    };
    changes.push(
