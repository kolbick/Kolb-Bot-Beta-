...
import { readConfigFileSnapshot, writeConfigFile, type KolbBotConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("kolb-bot config set gateway.auth.mode token")} or ${formatCliCommand("kolb-bot config set gateway.auth.mode password")}.`,
  ].join(" ");
}
