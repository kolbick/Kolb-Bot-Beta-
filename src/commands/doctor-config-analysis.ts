...
import { KolbBotSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: KolbBotConfig): {
  config: KolbBotConfig;
  removed: string[];
} {
  const parsed = KolbBotSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: KolbBotConfig): void {
