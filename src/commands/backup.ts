...
export async function backupCreateCommand(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Kolb-Bot config file was found to back up."
        : "No local Kolb-Bot state was found to back up.",
    );
  }
  ...
  if (!opts.dryRun) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-backup-"));
