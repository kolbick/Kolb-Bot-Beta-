...
describe("cleanup path removals", () => {
  ...
  it("removes state and only linked paths outside state", async () => {
    ...
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "kolb-bot-cleanup");
    await removeStateAndLinkedPaths(
      {
        stateDir: path.join(tmpRoot, "state"),
        configPath: path.join(tmpRoot, "state", "kolb-bot.json"),
    ...
    const joinedLogs = runtime.log.mock.calls
      ...
      .join("\n");
    expect(joinedLogs).toContain("/tmp/kolb-bot-cleanup/state");
    expect(joinedLogs).toContain("/tmp/kolb-bot-cleanup/oauth");
    expect(joinedLogs).not.toContain("kolb-bot.json");
  });

  it("removes every workspace directory", async () => {
    const runtime = createRuntimeMock();
    const workspaces = ["/tmp/kolb-bot-workspace-1", "/tmp/kolb-bot-workspace-2"];

    await removeWorkspaceDirs(workspaces, runtime, { dryRun: true });

    const logs = runtime.log.mock.calls.map(([line]) => line);
    expect(logs).toContain("[dry-run] remove /tmp/kolb-bot-workspace-1");
    expect(logs).toContain("[dry-run] remove /tmp/kolb-bot-workspace-2");
  });
});
