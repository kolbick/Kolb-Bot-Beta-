...
function prependInternalEventContext(
  ...
  if (body.includes("Kolb-Bot runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("kolb-bot agents list")}" to see configured agents.`,
      );
    }
  }
