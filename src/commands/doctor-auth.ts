...
function pruneAuthProfiles(
  cfg: KolbBotConfig,
  profileIds: Set<string>,
): { next: KolbBotConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("kolb-bot models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("kolb-bot models auth setup-token")} or ${formatCliCommand(
      "kolb-bot configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "kolb-bot models auth login --provider openai-codex",
    )} or ${formatCliCommand("kolb-bot configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("kolb-bot configure")}\` or \`${formatCliCommand("kolb-bot onboard")}\`.`;
}
