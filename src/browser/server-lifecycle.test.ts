...
const { stopKolbBotChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopKolbBotChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopKolbBotChrome: stopKolbBotChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "kolb-bot", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopKolbBotChromeMock.mockClear();
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopKolbBotChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("kolb-bot browser stop failed: Error: oops");
  });
});
