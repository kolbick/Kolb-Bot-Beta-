...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredKolbBotTmpDir: vi.fn(() => "/tmp/kolb-bot"),
}));
vi.mock("../infra/tmp-kolb-bot-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredKolbBotTmpDir.mockReturnValue("/tmp/kolb-bot");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".kolb-bot-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredKolbBotTmpDir.mockReturnValue("/tmp/kolb-bot-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "kolb-bot-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "kolb-bot-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredKolbBotTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "kolb-bot-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "kolb-bot-preferred", "downloads")}${path.sep}`),
    );
  });
