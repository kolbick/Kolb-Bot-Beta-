...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".kolb-bot-profile-decorated");
}
 ...
 */
export function decorateKolbBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_KOLB_BOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_KOLB_BOT_BROWSER_COLOR).toUpperCase();
