...
const RELAY_TOKEN_CONTEXT = "kolb-bot-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const KOLB_BOT_RELAY_BROWSER = "Kolb-Bot/extension-relay";
...
async function resolveGatewayAuthToken(): Promise<string | null> {
  const envToken =
    process.env.KOLB_BOT_GATEWAY_TOKEN?.trim() || process.env.KOLB_BOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
    ...
    if (tokenRef) {
      ...
      throw new SecretRefUnavailableError(
        `extension relay requires a resolved gateway token, but gateway.auth.token SecretRef is unavailable (${refLabel}). Set KOLB_BOT_GATEWAY_TOKEN or resolve your secret provider.`,
      );
    }
...
export async function resolveRelayAcceptedTokensForPort(port: number): Promise<string[]> {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or KOLB_BOT_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedKolbBotRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === KOLB_BOT_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
