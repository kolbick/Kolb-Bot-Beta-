...
export function resolveKolbBotUserDataDir(profileName = DEFAULT_KOLB_BOT_BROWSER_PROFILE_NAME) {
...
export async function launchKolbBotChrome(
  ...
  const userDataDir = resolveKolbBotUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_KOLB_BOT_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateKolbBotProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`kolb-bot browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopKolbBotChrome(
