import type { BrowserConfig, BrowserProfileConfig, KolbBotConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_KOLB_BOT_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_KOLB_BOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "kolb-bot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_KOLB_BOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_KOLB_BOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Kolb-Bot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for kolbick/Kolb-Bot-Beta- CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: KolbBotConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_KOLB_BOT_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_KOLB_BOT_BROWSER_PROFILE_NAME]
        ? DEFAULT_KOLB_BOT_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "kolb-bot";
