...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("kolb-bot");
    expect(profile?.driver).toBe("kolb-bot");
  ...
  it("derives default ports from KOLB_BOT_GATEWAY_PORT when unset", () => {
    withEnv({ KOLB_BOT_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ KOLB_BOT_GATEWAY_PORT: undefined }, () => {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "kolb-bot");
  ...
  describe("default profile preference", () => {
    it("defaults to kolb-bot profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps kolb-bot default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("kolb-bot");
    ...
    it("keeps kolb-bot default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("kolb-bot");
    ...
    it("keeps kolb-bot default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
