...
vi.mock("./chrome.js", () => ({
  resolveKolbBotUserDataDir: vi.fn(() => "/tmp/kolb-bot-test/kolb-bot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveKolbBotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "kolb-bot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveKolbBotUserDataDir).mockReturnValue(userDataDir);
