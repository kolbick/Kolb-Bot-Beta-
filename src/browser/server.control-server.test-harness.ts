...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.KOLB_BOT_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/kolb-bot" }));
...
vi.mock("./chrome.js", () => ({
  ...
  launchKolbBotChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.KOLB_BOT_GATEWAY_PORT;
  process.env.KOLB_BOT_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.KOLB_BOT_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.KOLB_BOT_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.KOLB_BOT_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.KOLB_BOT_GATEWAY_PASSWORD;
  } else {
    process.env.KOLB_BOT_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
