...
const RELAY_AUTH_HEADER = "x-kolb-bot-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/Kolb-Bot-Extension-Relay",
            revision: "0",
            userAgent: "Kolb-Bot-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "Kolb-Bot/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedKolbBotRelay({
