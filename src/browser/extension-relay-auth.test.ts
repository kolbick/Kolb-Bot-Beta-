...
import {
  probeAuthenticatedKolbBotRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedKolbBotRelay({
    baseUrl,
    relayAuthHeader: "x-kolb-bot-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    prevGatewayToken = process.env.KOLB_BOT_GATEWAY_TOKEN;
  ...
  it("accepts authenticated kolb-bot relay probe responses", async () => {
