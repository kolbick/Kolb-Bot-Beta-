...
import type { ConfigFileSnapshot, KolbBotConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: KolbBotConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Kolb-Bot state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Kolb-Bot state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.KOLB_BOT_GATEWAY_TOKEN) || hasNonEmptyString(env.KOLB_BOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.KOLB_BOT_GATEWAY_PASSWORD) ||
    hasNonEmptyString(env.KOLB_BOT_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("kolb-bot security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.KOLB_BOT_GATEWAY_TOKEN) ||
    hasNonEmptyString(env.KOLB_BOT_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: KolbBotConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: KolbBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: KolbBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("kolb-bot status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("kolb-bot security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set KOLB_BOT_GATEWAY_TOKEN/KOLB_BOT_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("kolb-bot security audit --deep")}".`,
    });
  }
