...
function listAgents(cfg: KolbBotConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: KolbBotConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<KolbBotConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: KolbBotConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: KolbBotConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: KolbBotConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<KolbBotConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  KolbBotConfig,
  {
    bindingsRef: KolbBotConfig["bindings"];
    agentsRef: KolbBotConfig["agents"];
    sessionRef: KolbBotConfig["session"];
...
function resolveRouteCacheForConfig(cfg: KolbBotConfig): Map<string, ResolvedAgentRoute> {
