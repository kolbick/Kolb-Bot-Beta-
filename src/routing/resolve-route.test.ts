...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: KolbBotConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: KolbBotConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/kolb-bot-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<KolbBotConfig["bindings"]>[number];
