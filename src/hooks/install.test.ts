...
const fixtureRoot = path.join(os.tmpdir(), `kolb-bot-hook-install-${randomUUID()}`);
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"kolb-bot":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expectPathInstallFailureContains(result, "kolb-bot.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expectPathInstallFailureContains(
      result,
      "kolb-bot.hooks entry resolves outside package directory",
    );
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@kolb-bot/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@kolb-bot/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@kolb-bot/test-hooks@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@kolb-bot/test-hooks@0.0.2-beta.1",
      name: "@kolb-bot/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@kolb-bot/test-hooks@beta"');
    }
  });
});
