...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => KolbBotConfig;
  action?: "new" | "reset";
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): KolbBotConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
