...
homepage: https://docs.github.com/kolbick/Kolb-Bot-Beta-/automation/hooks#command-logger
metadata:
  {
    "kolb-bot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.kolb-bot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.kolb-bot/logs/commands.log ~/.kolb-bot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/kolb-bot`:
   ```
   /home/username/.kolb-bot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
