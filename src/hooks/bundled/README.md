...
This directory contains hooks that ship with Kolb-Bot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.kolbick/Kolb-Bot-Beta-.json`:
...
2. Restart gateway: `pkill -9 -f 'kolb-bot.*gateway' && pnpm kolb-bot gateway`
3. Enable the hook: `kolb-bot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.github.com/kolbick/Kolb-Bot-Beta-/automation/hooks
