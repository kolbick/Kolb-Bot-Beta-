...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `kolb-bot-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["KOLB_BOT_GATEWAY_TOKEN", "KOLB_BOT_GATEWAY_PASSWORD"]);
  ...
  it.each([
    ...
    const result = await withEnvAsync({ KOLB_BOT_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it("falls back to KOLB_BOT_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers KOLB_BOT_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "KOLB_BOT_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-tui-file-secret-"));
