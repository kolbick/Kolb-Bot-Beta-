...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set KOLB_BOT_GATEWAY_TOKEN/KOLB_BOT_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "kolb-bot-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.KOLB_BOT_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.KOLB_BOT_GATEWAY_PASSWORD);
