...
describe("resolveInitialTuiAgentId", () => {
  const cfg: KolbBotConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/kolb-bot" },
        { id: "ops", workspace: "/tmp/kolb-bot/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/kolb-bot/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run kolb-bot devices list");
    expect(state.pairingHint).toContain("kolb-bot devices list");
  });
