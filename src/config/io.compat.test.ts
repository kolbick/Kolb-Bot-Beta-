...
describe("config io paths", () => {
  it("uses ~/.kolbick/Kolb-Bot-Beta-.json when config exists", async () => {
  ...
  it("defaults to ~/.kolbick/Kolb-Bot-Beta-.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
  ...
  it("honors explicit KOLB_BOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      ...
      const io = createIoForHome(home, { KOLB_BOT_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
      expect(io.configPath).toBe(customPath);
      expect(io.loadConfig().gateway?.port).toBe(20002);
    });
  });

  it("honors legacy KOLB_BOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".kolb-bot", 20003, "legacy-custom.json");
      const io = createIoForHome(home, { KOLB_BOT_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
      expect(io.configPath).toBe(customPath);
