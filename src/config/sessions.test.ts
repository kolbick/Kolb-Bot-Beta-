...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ KOLB_BOT_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-kolb-bot",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-kolb-bot#general");
  });
  ...
  it("derives session transcripts dir from KOLB_BOT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { KOLB_BOT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when KOLB_BOT_STATE_DIR differs from stored paths", () => {
