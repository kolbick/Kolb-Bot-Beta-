...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "kolb-bot.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.kolb-bot/workspace/canvas). */
  root?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/kolb-bot"). */
  basePath?: string;
 ...
 * Used when Kolb-Bot runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
