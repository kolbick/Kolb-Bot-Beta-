...
import type { ConfigFileSnapshot } from "./types.kolb-bot.js";
import { KolbBotSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(KolbBotSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.kolb-bot/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/kolb-bot/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/kolb-bot/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/kolb-bot/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/kolb-bot/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = KolbBotSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "KolbBotConfig";
    const hints = mainSchemaHints;
