...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("kolb-bot-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "kolb-bot-sandbox-browser-global" },
      agentBrowser: { network: "kolb-bot-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("kolb-bot-sandbox-browser-agent");
  });
