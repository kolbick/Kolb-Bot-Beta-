...
function isWhatsAppConfigured(cfg: KolbBotConfig): boolean {
...
function isGenericChannelConfigured(cfg: KolbBotConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: KolbBotConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: KolbBotConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: KolbBotConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: KolbBotConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: KolbBotConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: KolbBotConfig, pluginId: string): KolbBotConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
