...
function collectConfigEnvVarsByTarget(cfg?: KolbBotConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: KolbBotConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: KolbBotConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: KolbBotConfig): Record<string, string> {
...
export function applyConfigEnvVars(cfg: KolbBotConfig, env: NodeJS.ProcessEnv = process.env): void {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.KOLB_BOT_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
