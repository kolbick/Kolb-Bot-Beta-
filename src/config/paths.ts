...
/**
 * Nix mode detection: When KOLB_BOT_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.KOLB_BOT_NIX_MODE === "1";
}
...
/** Build a homedir thunk that respects KOLB_BOT_HOME for the given env. */
 ...
 * Can be overridden via KOLB_BOT_STATE_DIR.
 * Default: ~/.kolb-bot
 */
...
export function resolveConfigPath(
  ...
  const stateOverride = env.KOLB_BOT_STATE_DIR?.trim();
  const candidates = [
 ...
 * Default: os.tmpdir()/kolb-bot-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `kolb-bot-${uid}` : "kolb-bot";
 ...
 *
 * Precedence:
 * - `KOLB_BOT_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.KOLB_BOT_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
