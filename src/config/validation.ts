...
import type { KolbBotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: KolbBotConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: KolbBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: KolbBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = KolbBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as KolbBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as KolbBotConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as KolbBotConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as KolbBotConfig,
  };
}
