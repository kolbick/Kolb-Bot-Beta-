...
import type { KolbBotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareKolbBotVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  kolb-bot config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  kolb-bot config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: KolbBotConfig,
  pathSegments: string[],
): { changed: boolean; next: KolbBotConfig } {
...
function coerceConfig(value: unknown): KolbBotConfig {
...
function stampConfigVersion(cfg: KolbBotConfig): KolbBotConfig {
...
function warnIfConfigFromFuture(cfg: KolbBotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareKolbBotVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Kolb-Bot (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as KolbBotConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): KolbBotConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as KolbBotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: KolbBotConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as KolbBotConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.KOLB_BOT_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.KOLB_BOT_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.KOLB_BOT_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.KOLB_BOT_WATCH_SESSION === "string" &&
        deps.env.KOLB_BOT_WATCH_SESSION.trim().length > 0
          ? deps.env.KOLB_BOT_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.KOLB_BOT_WATCH_COMMAND === "string" &&
        deps.env.KOLB_BOT_WATCH_COMMAND.trim().length > 0
          ? deps.env.KOLB_BOT_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `KOLB_BOT_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: KolbBotConfig | null = null;
let runtimeConfigSourceSnapshot: KolbBotConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.KOLB_BOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.KOLB_BOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): KolbBotConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): KolbBotConfig | null {
...
export function loadConfig(): KolbBotConfig {
...
export async function readBestEffortConfig(): Promise<KolbBotConfig> {
...
export async function writeConfigFile(
  ...
  // edits to kolb-bot.json remain visible (no stale snapshot).
}
