...
import { buildWebSearchProviderConfig, withTempHome, writeKolbBotConfig } from "./test-helpers.js";
import { KolbBotSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = KolbBotSchema.safeParse({
      $schema: "https://github.com/kolbick/Kolb-Bot-Beta-/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://github.com/kolbick/Kolb-Bot-Beta-/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = KolbBotSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = KolbBotSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeKolbBotConfig(home, {
        gateway: { bind: "${KOLB_BOT_BIND}" },
      });

      const prev = process.env.KOLB_BOT_BIND;
      process.env.KOLB_BOT_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.KOLB_BOT_BIND;
        } else {
          process.env.KOLB_BOT_BIND = prev;
        }
      }
    });
  });
