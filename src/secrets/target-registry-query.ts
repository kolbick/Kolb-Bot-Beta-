...
const KOLB_BOT_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of KOLB_BOT_COMPILED_SECRET_TARGETS) {
...
const KOLB_BOT_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: KOLB_BOT_COMPILED_SECRET_TARGETS,
    entriesById: KOLB_BOT_TARGETS_BY_ID,
  });
