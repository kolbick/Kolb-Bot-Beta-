...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.KOLB_BOT_STATE_DIR;

    const tempHome = await createTempHomeEnv("kolb-bot-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.KOLB_BOT_STATE_DIR).toBe(path.join(tempHome.home, ".kolb-bot"));
    ...
    expect(process.env.KOLB_BOT_STATE_DIR).toBe(previousStateDir);
