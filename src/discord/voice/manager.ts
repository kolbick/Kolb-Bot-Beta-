...
function resolveVoiceTtsConfig(params: { cfg: KolbBotConfig; override?: TtsConfig }): {
  cfg: KolbBotConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredKolbBotTmpDir(), "discord-voice-"));
