  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/kolb-bot",
      },
    },
    session: { store: "/tmp/kolb-bot-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bkolb-bot\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "kolb-bot: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "Kolb-Bot" },
          },
        },
        eventPatch: {
