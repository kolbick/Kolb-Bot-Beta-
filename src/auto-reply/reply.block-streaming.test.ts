...
function createReplyConfig(home: string, streamMode?: "block"): KolbBotConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("kolb-bot-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("KOLB_BOT_TEST_FAST", "1");
  ...
  it("still parses media directives for direct block payloads", async () => {
    await withTempHome(async (home) => {
      ...
      expect(onBlockReply.mock.calls[0][0]).toMatchObject({
        text: "Result",
        mediaUrls: [path.join(home, "kolb-bot", "image.png")],
      });
    });
  });
});
