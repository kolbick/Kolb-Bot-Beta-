...
export function isCommandEnabled(cfg: KolbBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: KolbBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as KolbBotConfig),
...
export function getCommandDetection(_cfg?: KolbBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: KolbBotConfig) {
