...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop kolb-bot!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@kolb-bot_bot", { botUsername: "kolb-bot_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@kolb-bot_bot", { botUsername: "kolb-bot_bot" })).toBe(true);
