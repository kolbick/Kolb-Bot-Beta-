...
import { scheduleGatewaySigusr1Restart, triggerKolbBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Kolb-Bot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerKolbBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Kolb-Bot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
