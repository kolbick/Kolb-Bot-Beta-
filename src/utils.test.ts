...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("kolb-bot-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("kolb-bot-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("kolb-bot-lid-a-", (first) => {
      withTempDirSync("kolb-bot-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.kolb-bot when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "kolb-bot-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/kolb-bot")).toBe(path.resolve(os.homedir(), "kolb-bot"));
  });
