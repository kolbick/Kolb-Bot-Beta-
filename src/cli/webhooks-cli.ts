...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.github.com/kolbick/Kolb-Bot-Beta-/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Kolb-Bot hooks")
    ...
    .option("--hook-url <url>", "Kolb-Bot hook URL")
    .option("--hook-token <token>", "Kolb-Bot hook token")
