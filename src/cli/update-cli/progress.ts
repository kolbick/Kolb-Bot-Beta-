...
const STEP_LABELS: Record<string, string> = {
  ...
  "kolb-bot doctor entry": "Checking doctor entrypoint",
  "kolb-bot doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g kolb-bot@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g kolb-bot@latest --omit=optional");
  }
