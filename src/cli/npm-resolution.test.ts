...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@kolb-bot/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@kolb-bot/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@kolb-bot/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@kolb-bot/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @kolb-bot/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@kolb-bot/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@kolb-bot/plugin-alpha@1.2.3",
        installPath: "/tmp/kolb-bot/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@kolb-bot/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @kolb-bot/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@kolb-bot/plugin-alpha@latest",
      true,
      "/tmp/kolb-bot/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@kolb-bot/plugin-alpha@latest",
      installPath: "/tmp/kolb-bot/extensions/alpha",
      version: "1.2.3",
