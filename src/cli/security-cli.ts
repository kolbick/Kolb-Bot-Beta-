...
export function registerSecurityCli(program: Command) {
  const security = program
    ...
    .addHelpText(
      ...
      () =>
        ...
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.github.com/kolbick/Kolb-Bot-Beta-/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Kolb-Bot security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("kolb-bot security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("kolb-bot security audit --fix")}`));
        if (!fixResult) {
