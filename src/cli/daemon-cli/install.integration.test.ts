...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("kolb-bot-daemon-install-int-");
    configPath = path.join(tempHome, "kolb-bot.json");
    process.env.HOME = tempHome;
    process.env.KOLB_BOT_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.KOLB_BOT_GATEWAY_TOKEN).toBeUndefined();
  });
});
