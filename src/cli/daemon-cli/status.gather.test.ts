...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    KOLB_BOT_STATE_DIR: "/tmp/kolb-bot-daemon",
    KOLB_BOT_CONFIG_PATH: "/tmp/kolb-bot-daemon/kolb-bot.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.KOLB_BOT_STATE_DIR ?? "/tmp/kolb-bot-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.KOLB_BOT_CONFIG_PATH ?? `${stateDir}/kolb-bot.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/kolb-bot-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.KOLB_BOT_STATE_DIR = "/tmp/kolb-bot-cli";
    process.env.KOLB_BOT_CONFIG_PATH = "/tmp/kolb-bot-cli/kolb-bot.json";
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.KOLB_BOT_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
