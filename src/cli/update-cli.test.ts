...
import type { KolbBotConfig, ConfigFileSnapshot } from "../config/types.kolb-bot.js";
...
vi.mock("../infra/kolb-bot-root.js", () => ({
  resolveKolbBotPackageRoot: vi.fn(),
}));
...
const { resolveKolbBotPackageRoot } = await import("../infra/kolb-bot-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/kolb-bot-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as KolbBotConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/kolb-bot-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveKolbBotPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("kolb-bot-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveKolbBotPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("kolb-bot");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/kolb-bot-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "kolb-bot-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Kolb-Bot update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as KolbBotConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveKolbBotPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("kolb-bot-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ KOLB_BOT_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.KOLB_BOT_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("kolb-bot-update-wizard");
    await withEnvAsync({ KOLB_BOT_GIT_DIR: tempDir }, async () => {
      setTty(true);
