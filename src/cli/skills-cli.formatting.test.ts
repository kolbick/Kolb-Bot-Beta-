...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-bundled-skills-test-"));
    process.env.KOLB_BOT_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
