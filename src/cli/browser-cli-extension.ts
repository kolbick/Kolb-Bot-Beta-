...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall Kolb-Bot and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “Kolb-Bot Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.github.com/kolbick/Kolb-Bot-Beta-/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
        ...
        defaultRuntime.error(
          danger(
            [
              ...
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.github.com/kolbick/Kolb-Bot-Beta-/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
