    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kolb-bot memory status", "Show index and provider status."],
          ["kolb-bot memory status --deep", "Probe embedding provider readiness."],
          ["kolb-bot memory index --force", "Force a full reindex."],
          ['kolb-bot memory search "meeting notes"', "Quick search using positional query."],
          [
            'kolb-bot memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["kolb-bot memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.github.com/kolbick/Kolb-Bot-Beta-/cli/memory")}\n`,
    );
