...
function appendKolb-HubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("kolb-bot skills list")}\` to see all skills.`
      : "No skills found.";
    return appendKolb-HubHint(message, opts.json);
  }
  ...
  return appendKolb-HubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendKolb-HubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("kolb-bot skills list")}\` to see available skills.`,
      opts.json,
    );
  }
