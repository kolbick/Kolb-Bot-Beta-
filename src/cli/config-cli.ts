...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("kolb-bot doctor")}\` ${message}`;
}
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "kolb-bot.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.github.com/kolbick/Kolb-Bot-Beta-/cli/config")}\n`,
    )
    .option(
      "--section <section>",
