...
beforeEach(() => {
  ...
  originalHideBanner = process.env.KOLB_BOT_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.KOLB_BOT_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.KOLB_BOT_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("kolb-bot");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "kolb-bot", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("kolb-bot-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "kolb-bot", "message", "send"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "kolb-bot", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.KOLB_BOT_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "kolb-bot", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "kolb-bot", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "kolb-bot", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "kolb-bot", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "kolb-bot", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "kolb-bot", "backup", "create", "--json"],
    });
