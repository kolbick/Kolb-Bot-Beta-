...
${formatHelpExamples([
  ['kolb-bot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['kolb-bot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'kolb-bot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'kolb-bot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['kolb-bot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'kolb-bot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.github.com/kolbick/Kolb-Bot-Beta-/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.github.com/kolbick/Kolb-Bot-Beta-/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['kolb-bot agents set-identity --agent main --name "Kolb-Bot" --emoji "🏴‍☠️"', "Set name + emoji."],
  ...
  [
    "kolb-bot agents set-identity --identity-file ~/.kolb-bot/workspace/IDENTITY.md --agent main",
