...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Kolb-Bot and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["kolb-bot update", "Update a source checkout (git)"],
        ["kolb-bot update --channel beta", "Switch to beta channel (git + npm)"],
        ["kolb-bot update --channel dev", "Switch to dev channel (git + npm)"],
        ["kolb-bot update --tag beta", "One-off update to a dist-tag or version"],
        ["kolb-bot update --dry-run", "Preview actions without changing anything"],
        ["kolb-bot update --no-restart", "Update without restarting the service"],
        ["kolb-bot update --json", "Output result as JSON"],
        ["kolb-bot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["kolb-bot update wizard", "Interactive update wizard"],
        ["kolb-bot --update", "Shorthand for kolb-bot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run kolb-bot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.github.com/kolbick/Kolb-Bot-Beta-/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kolb-bot update status", "Show channel + version status."],
          ["kolb-bot update status --json", "JSON output."],
          ["kolb-bot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.github.com/kolbick/Kolb-Bot-Beta-/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
