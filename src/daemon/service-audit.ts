...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds KOLB_BOT_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `kolb-bot gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service KOLB_BOT_GATEWAY_TOKEN does not match gateway.auth.token in kolb-bot.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.KOLB_BOT_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.KOLB_BOT_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `kolb-bot gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
