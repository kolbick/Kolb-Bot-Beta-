...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `kolb-bot-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      KOLB_BOT_LAUNCHD_LABEL: `ai.kolb-bot.launchd-int-${testId}`,
      KOLB_BOT_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
