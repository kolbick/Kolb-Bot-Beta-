...
describe("buildPairingReply", () => {
  ...
  for (const testCase of cases) {
    it(`formats pairing reply for ${testCase.channel}`, () => {
      ...
      // CLI commands should respect KOLB_BOT_PROFILE when set (most tests run with isolated profile)
      const commandRe = new RegExp(
        `(?:kolb-bot|kolb-bot) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
      );
