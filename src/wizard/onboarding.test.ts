...
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.kolbick/Kolb-Bot-Beta-.json",
    exists: false,
...
vi.mock("../commands/onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "/tmp/kolb-bot-workspace",
...
describe("runOnboardingWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-onboard-suite-"));
  });
  ...
  it("resolves gateway.auth.password SecretRef for local onboarding probe", async () => {
    ...
    process.env.KOLB_BOT_GATEWAY_PASSWORD = "gateway-ref-password"; // pragma: allowlist secret
