...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Kolb-Bot is a hobby project and still in beta. Expect sharp edges.",
      "By default, Kolb-Bot is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Kolb-Bot is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Kolb-Bot.",
      ...
      "",
      "Must read: https://docs.github.com/kolbick/Kolb-Bot-Beta-/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Kolb-Bot onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: KolbBotConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.github.com/kolbick/Kolb-Bot-Beta-/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("kolb-bot doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("kolb-bot configure")}.`;
  ...
  let localGatewayToken = process.env.KOLB_BOT_GATEWAY_TOKEN ?? process.env.KOLB_BOT_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword =
    process.env.KOLB_BOT_GATEWAY_PASSWORD ?? process.env.KOLB_BOT_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: KolbBotConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
