...
const { createKolbBotTools } = await import("../agents/kolb-bot-tools.js");
...
type SessionSendTool = ReturnType<typeof createKolbBotTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createKolbBotTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["KOLB_BOT_GATEWAY_PORT", "KOLB_BOT_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.KOLB_BOT_GATEWAY_PORT = String(gatewayPort);
  process.env.KOLB_BOT_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("KOLB_BOT_CONFIG_PATH missing in gateway test environment");
      }
