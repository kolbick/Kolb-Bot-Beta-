...
export function resolveHooksConfig(cfg: KolbBotConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: KolbBotConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-kolb-bot-token"] === "string"
      ? req.headers["x-kolb-bot-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
