...
const gatewayEnv = {
  KOLB_BOT_GATEWAY_TOKEN: "env-token", // pragma: allowlist secret
  KOLB_BOT_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
} as NodeJS.ProcessEnv;

function makeRemoteGatewayConfig(remote: { token?: string; password?: string }): KolbBotConfig {
  return {
    gateway: {
      mode: "remote",
      remote,
...
describe("gateway credential precedence parity", () => {
  const cases: TestCase[] = [
    ...
    },
    {
      ...
      env: {
        KOLB_BOT_GATEWAY_TOKEN: "legacy-token", // pragma: allowlist secret
