...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.KOLB_BOT_GATEWAY_URL) ??
      trimToUndefined(process.env.KOLB_BOT_GATEWAY_URL));
  ...
  const allowPrivateWs = process.env.KOLB_BOT_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set KOLB_BOT_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: kolb-bot doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__KOLB_BOT_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
