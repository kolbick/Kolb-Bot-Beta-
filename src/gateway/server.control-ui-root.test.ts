...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-gateway-ui-hardlink-"));
  try {
    const packageRoot = path.join(tmp, "pnpm-global", "5", "node_modules", "kolb-bot");
    const controlUiRoot = path.join(packageRoot, "dist", "control-ui");
    await fs.mkdir(controlUiRoot, { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "kolb-bot" }),
    );
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global Kolb-Bot package control-ui", async () => {
