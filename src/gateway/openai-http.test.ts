    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "kolb-bot", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-kolb-bot-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-kolb-bot-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "kolb-bot", messages: [{ role: "user", content: "hi" }] },
          {
            "x-kolb-bot-agent-id": "beta",
            "x-kolb-bot-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Kolb-Bot.");
      }
