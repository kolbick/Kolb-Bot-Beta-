...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("kolb-bot doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with KOLB_BOT_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with KOLB_BOT_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.KOLB_BOT_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://kolb-bot-gateway.ai:18789",
      onConnectError,
    });
