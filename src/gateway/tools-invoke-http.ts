...
import { createKolbBotTools } from "../agents/kolb-bot-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-kolb-bot-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-kolb-bot-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-kolb-bot-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-kolb-bot-thread-id")?.trim() || undefined;
  ...
  const allTools = createKolbBotTools({
