...
let lastCreateKolbBotToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/kolb-bot-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateKolbBotToolsContext?.agentTo,
          agentThreadId: lastCreateKolbBotToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createKolbBotTools: (ctx: Record<string, unknown>) => {
      lastCreateKolbBotToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateKolbBotToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateKolbBotToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-kolb-bot-message-to": "channel:24514",
        "x-kolb-bot-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
