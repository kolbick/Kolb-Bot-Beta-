...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "kolb-bot");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = nextGatewayId("nonce-a");
      const nonceB = nextGatewayId("nonce-b");
      const toolProbePath = path.join(workspaceDir, `.kolb-bot-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".kolb-bot");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "kolb-bot.json");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-wizard-home-"));
      process.env.HOME = tempHome;
