...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "KOLB_BOT_SKIP_BROWSER_CONTROL_SERVER",
  "KOLB_BOT_SKIP_GMAIL_WATCHER",
  "KOLB_BOT_SKIP_CANVAS_HOST",
  "KOLB_BOT_BUNDLED_PLUGINS_DIR",
  "KOLB_BOT_SKIP_CHANNELS",
  "KOLB_BOT_SKIP_PROVIDERS",
  "KOLB_BOT_SKIP_CRON",
  "KOLB_BOT_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.KOLB_BOT_STATE_DIR = path.join(tempHome, ".kolb-bot");
  delete process.env.KOLB_BOT_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.KOLB_BOT_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.KOLB_BOT_SKIP_GMAIL_WATCHER = "1";
  process.env.KOLB_BOT_SKIP_CANVAS_HOST = "1";
  process.env.KOLB_BOT_SKIP_CHANNELS = "1";
  process.env.KOLB_BOT_SKIP_PROVIDERS = "1";
  process.env.KOLB_BOT_SKIP_CRON = "1";
  process.env.KOLB_BOT_TEST_MINIMAL_GATEWAY = "1";
  process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "kolb-bot-test-no-bundled-extensions")
    : "kolb-bot-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".kolb-bot-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".kolb-bot-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__kolbBotTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__kolbBotTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["KOLB_BOT_GATEWAY_TOKEN"]);
  const prev = process.env.KOLB_BOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.KOLB_BOT_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.KOLB_BOT_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
