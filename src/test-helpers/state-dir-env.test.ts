...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips KOLB_BOT_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/kolb-bot-state-dir-test");
    expect(process.env.KOLB_BOT_STATE_DIR).toBe("/tmp/kolb-bot-state-dir-test");
    expect(process.env.KOLB_BOT_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("kolb-bot-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.KOLB_BOT_STATE_DIR).toBe(stateDir);
      expect(process.env.KOLB_BOT_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("kolb-bot-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
  ...
  it("withStateDirEnv restores both env vars when legacy var was previously set", async () => {
    ...
    process.env.KOLB_BOT_STATE_DIR = "/tmp/original-kolb-bot";
    process.env.KOLB_BOT_STATE_DIR = "/tmp/original-legacy";
    const prev = snapshotCurrentStateDirVars();
