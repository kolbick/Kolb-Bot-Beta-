...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-install-sh-"));
  const cliPath = path.join(root, "kolb-bot");
...
KOLB_BOT_BIN="$FAKE_KOLB_BOT_BIN"
resolve_kolb-bot_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_KOLB_BOT_BIN: cliPath,
        KOLB_BOT_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Kolb-Bot 2026.3.9 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Kolb-Bot dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Kolb-Bot dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-install-stdin-"));
