...
describe("canvas host state dir defaults", () => {
  it("uses KOLB_BOT_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("kolb-bot-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Kolb-Bot Canvas");
      } finally {
        await handler.close();
      }
    });
