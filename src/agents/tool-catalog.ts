...
function buildCoreToolGroupMap() {
  ...
  const kolbBotTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInKolbBotGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:kolb-bot": kolbBotTools,
