 ...
 * A common mistake — caused by the similarity with the `kolb-bot.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (kolb-bot.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.KOLB_BOT_AUTH_STORE_READONLY === "1";
