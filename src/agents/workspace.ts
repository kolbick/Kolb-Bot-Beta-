...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.KOLB_BOT_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".kolb-bot", `workspace-${profile}`);
  }
  return path.join(home, ".kolb-bot", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".kolb-bot";
