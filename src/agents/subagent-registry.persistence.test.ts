...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["KOLB_BOT_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-subagent-"));
  ...
  it("uses isolated temp state when KOLB_BOT_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "kolb-bot-test-state"));
  });
});
