...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "kolb-bot-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.KOLB_BOT_BUNDLED_SKILLS_DIR = bundledDir;
