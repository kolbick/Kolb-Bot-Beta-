...
type WebSearchConfig = NonNullable<KolbBotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: KolbBotConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "brave") {
    return {
      ...
      message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("kolb-bot configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
