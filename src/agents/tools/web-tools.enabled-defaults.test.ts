...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "kolb-bot" }),
                      },
                    },
                  ],
                },
    ...
    const result = await tool?.execute?.("call-1", { query: "latest kolb-bot release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://github.com/kolbick/Kolb-Bot-Beta-/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://github.com/kolbick/Kolb-Bot-Beta-/docs"]);
