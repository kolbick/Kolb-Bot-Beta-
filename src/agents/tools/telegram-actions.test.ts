...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): KolbBotConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as KolbBotConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): KolbBotConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as KolbBotConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: KolbBotConfig, accountId = "media") {
    await handleTelegramAction(
