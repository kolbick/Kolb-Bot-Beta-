...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("kolb-bot-ws-", async (workspaceDir) => {
      await withTempDir("kolb-bot-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createKolbBotCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("kolb-bot-ws-", async (workspaceDir) => {
      await withTempDir("kolb-bot-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("kolb-bot-ws-", async (workspaceDir) => {
      const cfg: KolbBotConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createKolbBotCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-kolb-bot.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("kolb-bot-sandbox-", async (sandboxDir) => {
      await withTempDir("kolb-bot-workspace-", async (workspaceDir) => {
        ...
        const tools = createKolbBotCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
