...
const defaultTools = createKolbBotCodingTools({ senderIsOwner: true });
...
describe("createKolbBotCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: KolbBotConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createKolbBotCodingTools({
    ...
    const anthropicTools = createKolbBotCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: KolbBotConfig = {
      tools: {
        exec: {
    ...
    const allowed = createKolbBotCodingTools({
    ...
    const denied = createKolbBotCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createKolbBotCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
