...
const GOOGLE_LIVE_SINGLE_KEY = "KOLB_BOT_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "KOLB_BOT_LIVE_ANTHROPIC_KEY",
    listVar: "KOLB_BOT_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "KOLB_BOT_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `KOLB_BOT_LIVE_${base}_KEY`;
