...
import { resolvePreferredKolbBotTmpDir } from "../infra/tmp-kolb-bot-dir.js";
...
async function withOutsideHardlinkInKolbBotTmp<T>(
  params: {
    kolbBotTmpDir: string;
  ...
  const hardlinkPath = path.join(params.kolbBotTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.kolbBotTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.kolbBotTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.kolbBotTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const kolbBotTmpDir = resolvePreferredKolbBotTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Kolb-Bot tmp root",
      media: path.join(kolbBotTmpDir, "image.png"),
      expected: path.join(kolbBotTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Kolb-Bot tmp root",
      media: pathToFileURL(path.join(kolbBotTmpDir, "photo.png")).href,
      expected: path.join(kolbBotTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Kolb-Bot tmp root",
      media: path.join(kolbBotTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(kolbBotTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(kolbBotTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside kolb-bot tmp root",
      media: path.join(os.tmpdir(), "outside-kolb-bot", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Kolb-Bot tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(kolbBotTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(kolbBotTmpDir, { recursive: true });
      const symlinkPath = path.join(kolbBotTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Kolb-Bot tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInKolbBotTmp(
      {
        kolbBotTmpDir,
  ...
  it("rejects symlinked Kolb-Bot tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInKolbBotTmp(
