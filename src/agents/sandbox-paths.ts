...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const kolbBotTmpDir = path.resolve(resolvePreferredKolbBotTmpDir());
  if (!isPathInside(kolbBotTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: kolbBotTmpDir });
  return resolved;
}
