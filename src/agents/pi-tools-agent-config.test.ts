...
import { createKolbBotCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: KolbBotConfig) {
    return createKolbBotCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<KolbBotConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<KolbBotConfig["agents"]>["list"]>[number]["tools"];
  }): KolbBotConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createKolbBotCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createKolbBotCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createKolbBotCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createKolbBotCodingTools({
      config: cfg,
    ...
    const bobTools = createKolbBotCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createKolbBotCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createKolbBotCodingTools({
      config: cfg,
