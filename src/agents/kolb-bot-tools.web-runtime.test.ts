...
function findTool(name: string, config: KolbBotConfig) {
  const allTools = createKolbBotTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: KolbBotConfig; env?: NodeJS.ProcessEnv }) {
...
describe("kolb-bot tools runtime web metadata wiring", () => {
