...
function resolveAttachmentLimits(config: KolbBotConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".kolb-bot", "attachments");
  const relDir = path.posix.join(".kolb-bot", "attachments", attachmentId);
