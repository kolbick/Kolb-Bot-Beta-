...
describe("createKolbBotCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-read-"));
    try {
      ...
      const contents = "Hello from kolb-bot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createKolbBotCodingTools({ sandbox });
