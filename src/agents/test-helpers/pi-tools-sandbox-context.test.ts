...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("kolb-bot-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("kolb-bot-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("kolb-bot-sbx-");
