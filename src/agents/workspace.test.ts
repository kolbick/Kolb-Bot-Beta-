...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses KOLB_BOT_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      KOLB_BOT_HOME: "/srv/kolb-bot-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/kolb-bot-home"), ".kolb-bot", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".kolb-bot", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("kolb-bot-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-workspace-hardlink-"));
    try {
