...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: KolbBotConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/kolb-bot" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: KolbBotConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: KolbBotConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: KolbBotConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: KolbBotConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: KolbBotConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: KolbBotConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: KolbBotConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/kolb-bot");
  });

  it("uses KOLB_BOT_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "kolb-bot-home");
    vi.stubEnv("KOLB_BOT_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as KolbBotConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".kolb-bot", "workspace"));
  });

  it("uses KOLB_BOT_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "kolb-bot-home");
    vi.stubEnv("KOLB_BOT_HOME", home);
    // Clear state dir so it falls back to KOLB_BOT_HOME
    vi.stubEnv("KOLB_BOT_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as KolbBotConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".kolb-bot", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/kolb-bot-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/kolb-bot-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-agent-scope-"));
