...
const originalBundledDir = process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("kolb-bot-bundled-");
  const workspaceDir = await createTempDir("kolb-bot-workspace-");
...
afterEach(async () => {
  process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
