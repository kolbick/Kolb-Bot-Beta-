...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["KOLB_BOT_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns KOLB_BOT_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-bundled-override-"));
    process.env.KOLB_BOT_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.KOLB_BOT_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "kolb-bot" }));
