...
function resolveSkillsLimits(config?: KolbBotConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "kolb-bot-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "kolb-bot-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveKolbBotMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: KolbBotConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`kolb-bot skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
