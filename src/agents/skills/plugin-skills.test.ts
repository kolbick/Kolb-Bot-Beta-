    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "kolb-bot.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "kolb-bot.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "kolb-bot.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("kolb-bot-");
  const acpxRoot = await tempDirs.make("kolb-bot-acpx-plugin-");
  const helperRoot = await tempDirs.make("kolb-bot-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("kolb-bot-");
  const pluginRoot = await tempDirs.make("kolb-bot-plugin-");
  const outsideDir = await tempDirs.make("kolb-bot-outside-");
