...
import { discoverKolbBotPlugins } from "../../plugins/discovery.js";
import type { KolbBotPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, KolbBotPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["KOLB_BOT_PLUGIN_CATALOG_PATHS", "KOLB_BOT_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<KolbBotPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: KolbBotPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: KolbBotPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverKolbBotPlugins({ workspaceDir: options.workspaceDir });
