...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: KolbBotConfig, policy: ChannelAccessPolicy) => KolbBotConfig;
  resolveAllowlist: (params: { cfg: KolbBotConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: KolbBotConfig; resolved: TResolved }) => KolbBotConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: KolbBotConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: KolbBotConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: KolbBotConfig, policy: ChannelAccessPolicy): KolbBotConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: KolbBotConfig, policy: ChannelAccessPolicy): KolbBotConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: KolbBotConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: KolbBotConfig; resolved: string[] }) => {
      calls.push("apply");
