...
function telegramCfg(): KolbBotConfig {
  return { channels: { telegram: { botToken: "tok" } } } as KolbBotConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: KolbBotConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as KolbBotConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as KolbBotConfig;
...
function createSignalAccountOverrideCfg(): KolbBotConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as KolbBotConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as KolbBotConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as KolbBotConfig,
        accountId: undefined,
        params: {
