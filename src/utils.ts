...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".kolb-bot");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.KOLB_BOT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$KOLB_BOT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via KOLB_BOT_STATE_DIR.
