...
const { makeStorePath } = createCronStoreHarness({ prefix: "kolb-bot-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses KOLB_BOT_HOME for tilde expansion", () => {
    vi.stubEnv("KOLB_BOT_HOME", "/srv/kolb-bot-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/kolb-bot-home", "cron", "jobs.json"));
  });
});
