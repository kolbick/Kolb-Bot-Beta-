...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "kolb-bot-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".kolb-bot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<KolbBotConfig> = {},
): KolbBotConfig {
  const base: KolbBotConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<KolbBotConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<KolbBotConfig>,
