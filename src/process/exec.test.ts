...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        KOLB_BOT_BASE_ENV: "base",
        KOLB_BOT_TO_REMOVE: undefined,
      },
      env: {
        KOLB_BOT_TEST_ENV: "ok",
      },
    });

    expect(resolved.KOLB_BOT_BASE_ENV).toBe("base");
    expect(resolved.KOLB_BOT_TEST_ENV).toBe("ok");
    expect(resolved.KOLB_BOT_TO_REMOVE).toBeUndefined();
    expect(resolved.KOLB_BOT_CLI).toBe(KOLB_BOT_CLI_ENV_VALUE);
  });
