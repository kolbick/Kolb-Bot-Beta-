...
import { ensureKolbBotExecMarkerOnProcess } from "./infra/kolb-bot-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "kolb-bot.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "kolb-bot.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  process.title = "kolb-bot";
  ensureKolbBotExecMarkerOnProcess();
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.KOLB_BOT_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.KOLB_BOT_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.KOLB_BOT_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.KOLB_BOT_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[kolb-bot] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `Kolb-Bot ${VERSION} (${commit})` : `Kolb-Bot ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[kolb-bot] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[kolb-bot] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[kolb-bot] ${parsed.error}`);
      process.exit(2);
    }
