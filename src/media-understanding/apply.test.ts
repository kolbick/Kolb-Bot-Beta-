...
const TEMP_MEDIA_PREFIX = "kolb-bot-media-";
...
function createGroqAudioConfig(): KolbBotConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): KolbBotConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): KolbBotConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: KolbBotConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredKolbBotTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        KOLB_BOT_AGENT_DIR: isolatedAgentDir,
