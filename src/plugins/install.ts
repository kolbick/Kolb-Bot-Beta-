...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing kolb-bot.extensions; update the plugin package to include kolb-bot.extensions (for example ["./dist/index.js"]). See https://docs.github.com/kolbick/Kolb-Bot-Beta-/help/troubleshooting#plugin-install-fails-with-missing-kolb-bot-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_KOLB_BOT_EXTENSIONS: "missing_kolb-bot_extensions",
  EMPTY_KOLB_BOT_EXTENSIONS: "empty_kolb-bot_extensions",
...
function ensureKolbBotExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_KOLB_BOT_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json kolb-bot.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_KOLB_BOT_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureKolbBotExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from kolb-bot.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-kolb-bot"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "kolb-bot security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "kolb-bot security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "kolb-bot-npm-pack-",
    spec,
    timeoutMs,
