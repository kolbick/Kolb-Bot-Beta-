...
import { resolveKolbBotPackageRootSync } from "../infra/kolb-bot-root.js";
...
import { discoverKolbBotPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolveKolbBotPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`kolb-bot/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: KolbBotPluginDefinition;
  register?: KolbBotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as KolbBotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as KolbBotPluginDefinition;
...
export function loadKolbBotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "kolb-bot/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: KolbBotPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as KolbBotPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
