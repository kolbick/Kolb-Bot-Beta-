...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "kolb-bot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadKolbBotPlugins({
      cache: false,
...
function setupBundledTelegramPlugin() {
  ...
  process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadKolbBotPlugins>) {
...
function useNoBundledPlugins() {
  process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadKolbBotPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadKolbBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadKolbBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadKolbBotPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadKolbBotPlugins(options);
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "kolb-bot.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "kolb-bot.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("kolb-bot/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadKolbBotPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadKolbBotPlugins({
        cache: false,
