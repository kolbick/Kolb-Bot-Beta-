...
export type KolbBotPluginConfigSchema = {
...
export type KolbBotPluginToolContext = {
...
export type KolbBotPluginToolFactory = (
  ctx: KolbBotPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type KolbBotPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type KolbBotPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
  ...
  configPatch?: Partial<KolbBotConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type KolbBotPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current Kolb-Bot configuration */
  config: KolbBotConfig;
 ...
 */
export type KolbBotPluginCommandDefinition = {
...
export type KolbBotPluginHttpRouteAuth = "gateway" | "plugin";
export type KolbBotPluginHttpRouteMatch = "exact" | "prefix";

export type KolbBotPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type KolbBotPluginHttpRouteParams = {
  path: string;
  handler: KolbBotPluginHttpRouteHandler;
  auth: KolbBotPluginHttpRouteAuth;
  match?: KolbBotPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type KolbBotPluginCliContext = {
  program: Command;
  config: KolbBotConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type KolbBotPluginCliRegistrar = (ctx: KolbBotPluginCliContext) => void | Promise<void>;

export type KolbBotPluginServiceContext = {
  config: KolbBotConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type KolbBotPluginService = {
  id: string;
  start: (ctx: KolbBotPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: KolbBotPluginServiceContext) => void | Promise<void>;
};

export type KolbBotPluginChannelRegistration = {
  plugin: ChannelPlugin;
  dock?: ChannelDock;
};

export type KolbBotPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  kind?: PluginKind;
  configSchema?: KolbBotPluginConfigSchema;
  register?: (api: KolbBotPluginApi) => void | Promise<void>;
  activate?: (api: KolbBotPluginApi) => void | Promise<void>;
};

export type KolbBotPluginModule =
  | KolbBotPluginDefinition
  | ((api: KolbBotPluginApi) => void | Promise<void>);

export type KolbBotPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | KolbBotPluginToolFactory,
    opts?: KolbBotPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: KolbBotPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: KolbBotPluginHttpRouteParams) => void;
  registerChannel: (registration: KolbBotPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: KolbBotPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: KolbBotPluginService) => void;
   ...
   */
  registerCommand: (command: KolbBotPluginCommandDefinition) => void;
