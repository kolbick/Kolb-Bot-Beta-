...
import type {
  KolbBotPluginApi,
  KolbBotPluginChannelRegistration,
  KolbBotPluginCliRegistrar,
  KolbBotPluginCommandDefinition,
  KolbBotPluginHttpRouteAuth,
  KolbBotPluginHttpRouteMatch,
  KolbBotPluginHttpRouteHandler,
  KolbBotPluginHttpRouteParams,
  KolbBotPluginHookOptions,
  ProviderPlugin,
  KolbBotPluginService,
  KolbBotPluginToolContext,
...
export type PluginToolRegistration = {
  pluginId: string;
  factory: KolbBotPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
};
...
export type PluginCliRegistration = {
  pluginId: string;
  register: KolbBotPluginCliRegistrar;
  commands: string[];
  source: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  service: KolbBotPluginService;
  source: string;
};

export type PluginCommandRegistration = {
  pluginId: string;
  command: KolbBotPluginCommandDefinition;
  source: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: KolbBotPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: KolbBotPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: KolbBotPluginHookOptions | undefined,
    config: KolbBotPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: KolbBotPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: KolbBotPluginChannelRegistration | ChannelPlugin,
  ) => {
    const normalized =
      typeof (registration as KolbBotPluginChannelRegistration).plugin === "object"
        ? (registration as KolbBotPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: KolbBotPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: KolbBotPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: KolbBotPluginCommandDefinition) => {
