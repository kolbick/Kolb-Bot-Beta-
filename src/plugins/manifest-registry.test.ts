...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `kolb-bot-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "kolb-bot.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "kolb-bot.plugin.json");
  const linkedManifest = path.join(rootDir, "kolb-bot.plugin.json");
