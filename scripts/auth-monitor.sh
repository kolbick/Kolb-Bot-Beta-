...
#
# Suggested cron: */30 * * * * /home/admin/kolb-bot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Kolb-Bot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., kolb-bot-alerts)
...
STATE_FILE="$HOME/.kolb-bot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Kolb-Bot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use kolb-bot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Kolb-Bot to $NOTIFY_PHONE..."
            kolb-bot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Kolb-Bot is down. Run: ssh l36 '~/kolb-bot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
