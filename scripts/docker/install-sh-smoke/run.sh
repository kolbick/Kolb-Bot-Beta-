...
INSTALL_URL="${KOLB_BOT_INSTALL_URL:-https://kolb-bot.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${KOLB_BOT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${KOLB_BOT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="kolb-bot"
PACKAGE_NAME="${KOLB_BOT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (KOLB_BOT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${KOLB_BOT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${KOLB_BOT_INSTALL_LATEST_OUT:-}"
fi
