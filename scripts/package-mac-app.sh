...
# Build and bundle Kolb-Bot into a minimal .app we can open.
# Outputs to dist/Kolb-Bot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Kolb-Bot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Kolb-Bot"
BUNDLE_ID="${BUNDLE_ID:-ai.kolb-bot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/kolbick/Kolb-Bot-Beta-/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Kolb-Bot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :Kolb-BotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :Kolb-BotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Kolb-Bot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Kolb-Bot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Kolb-Bot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Kolb-Bot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Kolb-Bot/Resources/Kolb-Bot.icns" "$APP_ROOT/Contents/Resources/Kolb-Bot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Kolb-Bot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying KolbBotKit resources"
KOLBBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/KolbBotKit_KolbBotKit.bundle"
if [ -d "$KOLBBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/KolbBotKit_KolbBotKit.bundle"
  cp -R "$KOLBBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/KolbBotKit_KolbBotKit.bundle"
else
  echo "WARN: KolbBotKit resource bundle not found at $KOLBBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Kolb-Bot"
killall -q Kolb-Bot 2>/dev/null || true
