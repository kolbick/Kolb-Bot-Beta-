#!/bin/bash
# Setup Kolb-Bot Auth Management System
...
echo "=== Kolb-Bot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'kolb-bot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Kolb-Bot message: Send warning via Kolb-Bot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/kolb-bot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/kolb-bot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/kolb-bot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now kolb-bot-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/KolbAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/KolbAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Kolb*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (KolbAuth) shows status and opens auth URL if needed."
echo "The full widget (KolbAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status kolb-bot-auth-monitor.timer"
echo ""
echo "Quick commands:"
