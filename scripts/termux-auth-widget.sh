...
# Kolb-Bot Auth Widget for Termux
...
SERVER="${KOLB_BOT_SERVER:-${KOLB_BOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Kolb-Bot auth..."

STATUS=$(ssh "$SERVER" '$HOME/kolb-bot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/kolb-bot/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|KOLB_BOT_EXPIRING|KOLB_BOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/kolb-bot/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|KOLB_BOT_EXPIRED|KOLB_BOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Kolb-Bot won't work until re-auth"
                ;;
        esac
        ;;
