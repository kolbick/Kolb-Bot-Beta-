...
type DriverMode = "token" | "webhook" | "kolb-bot";
...
function resolveStateDir(): string {
  const override = process.env.KOLB_BOT_STATE_DIR?.trim() || process.env.KOLB_BOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.KOLB_BOT_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".kolb-bot");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver kolb-bot] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Kolb-Bot spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|kolb-bot> Driver transport mode (default: token)\n" +
    ...
    "  --kolb-bot-bin <path>        Kolb-Bot CLI binary for driver=kolb-bot (default: kolb-bot)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  KOLB_BOT_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  KOLB_BOT_DISCORD_SMOKE_DRIVER\n" +
    "  KOLB_BOT_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  KOLB_BOT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  KOLB_BOT_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  KOLB_BOT_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  KOLB_BOT_DISCORD_SMOKE_AGENT\n" +
    "  KOLB_BOT_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  KOLB_BOT_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  KOLB_BOT_DISCORD_SMOKE_POLL_MS\n" +
    "  KOLB_BOT_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  KOLB_BOT_DISCORD_SMOKE_KOLB_BOT_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.KOLB_BOT_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.KOLB_BOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.KOLB_BOT_DISCORD_SMOKE_DRIVER ||
    process.env.KOLB_BOT_DISCORD_SMOKE_DRIVER ||
    "token";
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.KOLB_BOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.KOLB_BOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.KOLB_BOT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.KOLB_BOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.KOLB_BOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.KOLB_BOT_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.KOLB_BOT_DISCORD_SMOKE_AGENT ||
    process.env.KOLB_BOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.KOLB_BOT_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.KOLB_BOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.KOLB_BOT_DISCORD_SMOKE_INSTRUCTION ||
    process.env.KOLB_BOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.KOLB_BOT_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.KOLB_BOT_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
...
async function kolbBotCliJson<T>(params: { kolbBotBin: string; args: string[] }): Promise<T> {
  ...
  if (!stdout) {
    throw new Error(`kolb-bot ${params.args.join(" ")} returned empty stdout`);
  }
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `kolb-bot-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("kolb-bot message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Kolb-Bot.`,
        diagnostics: {
          bindingCandidates: [
            {
