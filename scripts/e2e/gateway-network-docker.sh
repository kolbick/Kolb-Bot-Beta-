...
IMAGE_NAME="kolb-bot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="kolb-bot-net-e2e-$$"
GW_NAME="kolb-bot-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "KOLB_BOT_GATEWAY_TOKEN=$TOKEN" \
  -e "KOLB_BOT_SKIP_CHANNELS=1" \
  -e "KOLB_BOT_SKIP_GMAIL_WATCHER=1" \
  -e "KOLB_BOT_SKIP_CRON=1" \
  -e "KOLB_BOT_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
