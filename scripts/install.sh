...
# Kolb-Bot Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://github.com/kolbick/Kolb-Bot-Beta-/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Kolb-Bot."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${KOLB_BOT_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🏴‍☠️ Kolb-Bot Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://github.com/kolbick/Kolb-Bot-Beta-/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$KOLB_BOT_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.github.com/kolbick/Kolb-Bot-Beta-/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_kolb-bot_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.kolb-bot-* "$npm_root"/kolb-bot 2>/dev/null || true
}

extract_kolb-bot_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_kolb-bot_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/kolb-bot" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/kolb-bot"|"/usr/local/bin/kolb-bot")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/kolb-bot/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale kolb-bot symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing kolb-bot binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Kolb-Bot package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_kolb-bot_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Kolb-Bot npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*kolb-bot" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_kolb-bot_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_kolb-bot_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Kolb-Bot npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an kolb-bot binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "KOLB_BOT_TAGLINE_INDEX" "KOLB_BOT_TAGLINE_INDEX"
map_legacy_env "KOLB_BOT_NO_ONBOARD" "KOLB_BOT_NO_ONBOARD"
map_legacy_env "KOLB_BOT_NO_PROMPT" "KOLB_BOT_NO_PROMPT"
map_legacy_env "KOLB_BOT_DRY_RUN" "KOLB_BOT_DRY_RUN"
map_legacy_env "KOLB_BOT_INSTALL_METHOD" "KOLB_BOT_INSTALL_METHOD"
map_legacy_env "KOLB_BOT_VERSION" "KOLB_BOT_VERSION"
map_legacy_env "KOLB_BOT_BETA" "KOLB_BOT_BETA"
map_legacy_env "KOLB_BOT_GIT_DIR" "KOLB_BOT_GIT_DIR"
map_legacy_env "KOLB_BOT_GIT_UPDATE" "KOLB_BOT_GIT_UPDATE"
map_legacy_env "KOLB_BOT_NPM_LOGLEVEL" "KOLB_BOT_NPM_LOGLEVEL"
map_legacy_env "KOLB_BOT_VERBOSE" "KOLB_BOT_VERBOSE"
map_legacy_env "KOLB_BOT_PROFILE" "KOLB_BOT_PROFILE"
map_legacy_env "KOLB_BOT_INSTALL_SH_NO_RUN" "KOLB_BOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${KOLB_BOT_TAGLINE_INDEX:-}" ]]; then
        if [[ "${KOLB_BOT_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((KOLB_BOT_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${KOLB_BOT_NO_ONBOARD:-0}
NO_PROMPT=${KOLB_BOT_NO_PROMPT:-0}
DRY_RUN=${KOLB_BOT_DRY_RUN:-0}
INSTALL_METHOD=${KOLB_BOT_INSTALL_METHOD:-}
KOLB_BOT_VERSION=${KOLB_BOT_VERSION:-latest}
USE_BETA=${KOLB_BOT_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/kolb-bot"
GIT_DIR=${KOLB_BOT_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${KOLB_BOT_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${KOLB_BOT_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${KOLB_BOT_VERBOSE:-0}"
KOLB_BOT_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Kolb-Bot installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://github.com/kolbick/Kolb-Bot-Beta-/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/kolb-bot)
...
Environment variables:
  KOLB_BOT_INSTALL_METHOD=git|npm
  KOLB_BOT_VERSION=latest|next|<semver>
  KOLB_BOT_BETA=0|1
  KOLB_BOT_GIT_DIR=...
  KOLB_BOT_GIT_UPDATE=0|1
  KOLB_BOT_NO_PROMPT=1
  KOLB_BOT_DRY_RUN=1
  KOLB_BOT_NO_ONBOARD=1
  KOLB_BOT_VERBOSE=1
  KOLB_BOT_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://github.com/kolbick/Kolb-Bot-Beta-/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://github.com/kolbick/Kolb-Bot-Beta-/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://github.com/kolbick/Kolb-Bot-Beta-/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Kolb-Bot checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Kolb-Bot source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_kolb-bot_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"kolb-bot"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://github.com/kolbick/Kolb-Bot-Beta-/install.sh | bash"
}
...
ensure_kolb-bot_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/kolb-bot" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/kolb-bot" ]]; then
        ln -sf "$npm_root/kolb-bot/dist/entry.js" "${npm_bin}/kolb-bot"
        ui_info "Created kolb-bot bin link at ${npm_bin}/kolb-bot"
    fi
    return 0
}

# Check for existing Kolb-Bot installation
check_existing_kolb-bot() {
    if [[ -n "$(type -P kolb-bot 2>/dev/null || true)" ]]; then
        ui_info "Existing Kolb-Bot installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make kolb-bot show as \"command not found\" in new terminals."
...
warn_kolb-bot_not_found() {
    ui_warn "Installed, but kolb-bot is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t kolb-bot 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named kolb-bot; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_kolb-bot_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P kolb-bot 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/kolb-bot" ]]; then
        echo "${npm_bin}/kolb-bot"
        return 0
    fi
...
install_kolb-bot_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/kolbick/Kolb-Bot-Beta-.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Kolb-Bot from git checkout: ${repo_dir}"
    else
        ui_info "Installing Kolb-Bot from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Kolb-Bot" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Kolb-Bot" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/kolb-bot" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/kolb-bot"
    ui_success "Kolb-Bot wrapper installed to \$HOME/.local/bin/kolb-bot"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view kolb-bot dist-tags.beta 2>/dev/null || true)"
...
install_kolb-bot() {
    local package_name="kolb-bot"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            KOLB_BOT_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="kolb-bot"
        else
            KOLB_BOT_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${KOLB_BOT_VERSION}" ]]; then
        KOLB_BOT_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${KOLB_BOT_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Kolb-Bot v${resolved_version}"
    else
        ui_info "Installing Kolb-Bot (${KOLB_BOT_VERSION})"
    fi
    local install_spec=""
    if [[ "${KOLB_BOT_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${KOLB_BOT_VERSION}"
    fi

    if ! install_kolb-bot_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_kolb-bot_paths
        install_kolb-bot_npm "${install_spec}"
    fi

    if [[ "${KOLB_BOT_VERSION}" == "latest" && "${package_name}" == "kolb-bot" ]]; then
        if ! resolve_kolb-bot_bin &> /dev/null; then
            ui_warn "npm install kolb-bot@latest failed; retrying kolb-bot@next"
            cleanup_npm_kolb-bot_paths
            install_kolb-bot_npm "kolb-bot@next"
        fi
    fi

    ensure_kolb-bot_bin_link || true

    ui_success "Kolb-Bot installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${KOLB_BOT_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_kolb-bot_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (kolb-bot not on PATH yet)"
        warn_kolb-bot_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${KOLB_BOT_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.kolb-bot/workspace-${profile}"
    else
        echo "${HOME}/.kolb-bot/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${KOLB_BOT_CONFIG_PATH:-$HOME/.kolbick/Kolb-Bot-Beta-.json}"
    if [[ -f "${config_path}" || -f "$HOME/.kolb-bot/kolb-bot.json" || -f "$HOME/.kolb-bot/kolb-bot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run kolb-bot onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but kolb-bot not on PATH; skipping onboarding"
        warn_kolb-bot_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run kolb-bot onboard to retry"
        return
    }
}
...
resolve_kolb-bot_version() {
    local version=""
    local raw_version_output=""
    local claw="${KOLB_BOT_BIN:-}"
    if [[ -z "$claw" ]] && command -v kolb-bot &> /dev/null; then
        claw="$(command -v kolb-bot)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_kolb-bot_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/kolb-bot/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/kolb-bot/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_kolb-bot_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Kolb-Bot checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set KOLB_BOT_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_kolb-bot; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Kolb-Bot"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g kolb-bot &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g kolb-bot 2>/dev/null || true
        ...
        install_kolb-bot_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/kolb-bot" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/kolb-bot"
    ...
    KOLB_BOT_BIN="$(resolve_kolb-bot_bin || true)"
    ...
    installed_version=$(resolve_kolb-bot_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🏴‍☠️ Kolb-Bot installed successfully (${installed_version})!"
    else
        ui_celebrate "🏴‍☠️ Kolb-Bot installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/kolb-bot"
        ui_kv "Update command" "kolb-bot update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://github.com/kolbick/Kolb-Bot-Beta-/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running kolb-bot doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                KOLB_BOT_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                KOLB_BOT_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                KOLB_BOT_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run kolb-bot doctor and kolb-bot plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run kolb-bot onboard later"
        else
            local config_path="${KOLB_BOT_CONFIG_PATH:-$HOME/.kolbick/Kolb-Bot-Beta-.json}"
            if [[ -f "${config_path}" || -f "$HOME/.kolb-bot/kolb-bot.json" || -f "$HOME/.kolb-bot/kolb-bot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (kolb-bot not on PATH yet)"
                    warn_kolb-bot_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run kolb-bot onboard to finish setup"
            return 0
        fi
    fi

    if command -v kolb-bot &> /dev/null; then
        local claw="${KOLB_BOT_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_kolb-bot_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (kolb-bot daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if KOLB_BOT_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: kolb-bot daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${KOLB_BOT_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
