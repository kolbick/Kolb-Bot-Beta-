...
BASE_IMAGE="${BASE_IMAGE:-kolb-bot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-kolb-bot-sandbox-common:bookworm-slim}"
...
KOLB_BOT_DOCKER_BUILD_USE_BUILDX="${KOLB_BOT_DOCKER_BUILD_USE_BUILDX:-0}"
KOLB_BOT_DOCKER_BUILD_CACHE_FROM="${KOLB_BOT_DOCKER_BUILD_CACHE_FROM:-}"
KOLB_BOT_DOCKER_BUILD_CACHE_TO="${KOLB_BOT_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${KOLB_BOT_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${KOLB_BOT_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${KOLB_BOT_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${KOLB_BOT_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${KOLB_BOT_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=kolb-bot.sandbox=1)
NOTE
