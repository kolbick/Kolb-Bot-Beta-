...
const shouldBuild = (deps) => {
  if (deps.env.KOLB_BOT_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.KOLB_BOT_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[kolb-bot] ${message}\n`);
};

const runKolb-Bot = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["kolb-bot.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
