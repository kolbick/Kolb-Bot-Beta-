 ...
 * `kolb-bot update`. Use it to verify the completion installation flow
...
\`kolb-bot update\`, \`kolb-bot doctor\`, and \`kolb-bot onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Kolb-Bot Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`kolb-bot completion --install\` later to enable.`));
    return;
  }
