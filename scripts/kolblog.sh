...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "kolblog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
kolblog - Kolb-Bot Logging Utility

USAGE:
    kolblog [OPTIONS]

DESCRIPTION:
    View Kolb-Bot logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Kolb-Bot logs flow through the macOS unified log (subsystem: ai.kolb-bot).
...
QUICK START:
    kolblog -n 100             Show last 100 lines from all components
    kolblog -f                 Follow logs in real-time
    kolblog -e                 Show only errors
    kolblog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    kolblog                   Show last 50 lines from past 5 minutes (default)
    kolblog -f                Stream logs continuously
    kolblog -n 100            Show last 100 lines
    kolblog -e                Show only recent errors
    kolblog -l 30m -n 200     Show last 200 lines from past 30 minutes
    kolblog -c ServerManager  Show recent ServerManager logs
    kolblog -s "fail"         Search for "fail" in recent logs
    kolblog --server -e       Show recent server errors
    kolblog -f -d             Stream debug logs continuously
