---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Kolb-Bot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Kolb-Bot cron scheduling for periodic checks, or version status checks on a machine running Kolb-Bot (laptop, workstation, Pi, VPS).
---

# Kolb-Bot Host Hardening

## Overview

Assess and harden the host running Kolb-Bot, then align it to a user-defined risk tolerance without breaking access. Use Kolb-Bot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Kolb-Bot changes the host firewall, SSH, or OS updates; it does not.
...
5. Kolb-Bot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Kolb-Bot can access sensitive data.
...
### 2) Run Kolb-Bot security audits (read-only)

As part of the default read-only checks, run `kolb-bot security audit --deep`. Only offer alternatives if the user requests them:

1. `kolb-bot security audit` (faster, non-probing)
2. `kolb-bot security audit --json` (structured output)

Offer to apply Kolb-Bot safe defaults (numbered):

1. `kolb-bot security audit --fix`

Be explicit that `--fix` only tightens Kolb-Bot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Kolb-Bot version/update status (read-only)

As part of the default read-only checks, run `kolb-bot update status`.
...
- Credential hygiene notes (location of Kolb-Bot creds, prefer disk encryption)
...
- Kolb-Bot security audit (re-run)
...
After Kolb-Bot install or first hardening pass, run at least one baseline audit and version check:

- `kolb-bot security audit`
- `kolb-bot security audit --deep`
- `kolb-bot update status`

Ongoing monitoring is recommended. Use the Kolb-Bot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `kolb-bot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `kolb-bot update status`
...
Before creating, `kolb-bot cron list` and match on exact `name`. If found, `kolb-bot cron edit <id> ...`.
If not found, `kolb-bot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `kolb-bot update status` (preferred for source checkouts and channels)
2. `npm view kolb-bot version` (published npm version)

## Kolb-Bot command accuracy

Use only supported commands and flags:

- `kolb-bot security audit [--deep] [--fix] [--json]`
- `kolb-bot status` / `kolb-bot status --deep`
- `kolb-bot health --json`
- `kolb-bot update status`
- `kolb-bot cron add|list|runs|run`

Do not invent CLI flags or imply Kolb-Bot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Kolb-Bot compaction:
