...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `kolb-bot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".kolb-bot");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          KOLB_BOT_SKIP_PROVIDERS: "1",
          KOLB_BOT_SKIP_GMAIL_WATCHER: "1",
          KOLB_BOT_SKIP_CRON: "1",
          KOLB_BOT_SKIP_BROWSER_CONTROL_SERVER: "1",
          KOLB_BOT_SKIP_CANVAS_HOST: "1",
          KOLB_BOT_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
