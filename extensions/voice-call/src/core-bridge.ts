...
function resolveKolbBotRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.KOLB_BOT_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set KOLB_BOT_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveKolbBotRoot(), "dist", "extensionAPI.js");
