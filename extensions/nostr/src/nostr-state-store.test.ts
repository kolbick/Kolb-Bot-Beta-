...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.KOLB_BOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-nostr-"));
  process.env.KOLB_BOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.KOLB_BOT_STATE_DIR?.trim() || stateEnv.KOLB_BOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".kolb-bot");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.KOLB_BOT_STATE_DIR = previous;
    }
