...
import type { KolbBotPluginApi, KolbBotPluginService } from "kolb-bot/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: KolbBotPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: KolbBotPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.allowCommands ?? [])]);
}

function patchConfigNodeLists(
  cfg: KolbBotPluginApi["config"],
  next: { allowCommands: string[]; denyCommands: string[] },
): KolbBotPluginApi["config"] {
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
...
export default function register(api: KolbBotPluginApi) {
  ...
  const timerService: KolbBotPluginService = {
