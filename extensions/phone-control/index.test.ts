...
} from "kolb-bot/plugin-sdk/phone-control";
...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-phone-control-test-"));
    try {
      ...
      let command: KolbBotPluginCommandDefinition | undefined;
