...
import type { KolbBotPluginApi, KolbBotPluginToolContext } from "kolb-bot/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<KolbBotPluginApi> = {}): KolbBotPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<KolbBotPluginToolContext> = {}): KolbBotPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: KolbBotPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
