# Google Gemini CLI Auth (Kolb-Bot plugin)
...
```bash
kolb-bot plugins enable google-gemini-cli-auth
```
...
```bash
kolb-bot models auth login --provider google-gemini-cli --set-default
```
...
- `KOLB_BOT_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `KOLB_BOT_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
