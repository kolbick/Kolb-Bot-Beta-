...
import {
  ...
  type WizardPrompter,
} from "kolb-bot/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: KOLB_BOT_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: KolbBotConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var KOLB_BOT_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.KOLB_BOT_TWITCH_ACCESS_TOKEN?.trim();
