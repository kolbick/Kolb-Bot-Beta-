import type { RuntimeEnv, ReplyPayload, KolbBotConfig } from "kolb-bot/plugin-sdk/tlon";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "kolb-bot/plugin-sdk/tlon";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as KolbBotConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in Kolb-Bot config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your Kolb-Bot config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.github.com/kolbick/Kolb-Bot-Beta-/concepts/session#secure-dm-mode`;
