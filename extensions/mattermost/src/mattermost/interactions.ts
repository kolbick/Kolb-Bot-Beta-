...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__kolb-bot_channel_id";
...
type InteractionCallbackConfig = Pick<KolbBotConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "kolb-bot-mattermost-interactions").update(botToken).digest("hex");
}
