import type { AnyAgentTool, KolbBotPluginApi } from "kolb-bot/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: KolbBotPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<KolbBotPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
