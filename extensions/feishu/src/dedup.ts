...
} from "kolb-bot/plugin-sdk/feishu";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.KOLB_BOT_STATE_DIR?.trim() || env.KOLB_BOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["kolb-bot-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".kolb-bot");
}
