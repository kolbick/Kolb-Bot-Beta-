...
vi.mock("kolb-bot/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("kolb-bot/plugin-sdk/diagnostics-otel")>(
    "kolb-bot/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { KolbBotPluginServiceContext } from "kolb-bot/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "kolb-bot/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/kolb-bot-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): KolbBotPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("kolb-bot.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("kolb-bot.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("kolb-bot.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("kolb-bot.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("kolb-bot.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("kolb-bot.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("kolb-bot.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("kolb-bot.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("kolb-bot.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("kolb-bot.webhook.processed");
    expect(spanNames).toContain("kolb-bot.message.processed");
    expect(spanNames).toContain("kolb-bot.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["kolb-bot.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("kolb-bot.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "kolb-bot.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["kolb-bot.reason"]).toBe("string");
    expect(String(attrs?.["kolb-bot.reason"])).not.toContain(
