...
Read-only diff viewer plugin for **Kolb-Bot** agents.
...
Set plugin-wide defaults in `~/.kolbick/Kolb-Bot-Beta-.json`:
...
Before:
Kolb-Bot supports plugins.

After:
Kolb-Bot supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/kolb-bot-diffs`).
