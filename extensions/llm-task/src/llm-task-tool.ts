...
import Ajv from "ajv";
import { resolvePreferredKolbBotTmpDir } from "kolb-bot/plugin-sdk/llm-task";
// NOTE: This extension is intended to be bundled with Kolb-Bot.
// When running from source (tests/dev), Kolb-Bot internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { KolbBotPluginApi } from "kolb-bot/plugin-sdk/llm-task";
...
export function createLlmTaskTool(api: KolbBotPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via kolb-bot.invoke.",
