...
`kolb-bot.invoke --each`) without adding new Kolb-Bot code per workflow.
...
This extension depends on Kolb-Bot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Kolb-Bot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.kolb-bot/extensions` as a standalone plugin directory.
