...
import type { PluginRuntime, RuntimeEnv, RuntimeLogger } from "kolb-bot/plugin-sdk/matrix";
  ...
  it("stores sender-labeled BodyForAgent for group thread messages", async () => {
    ...
    const core = {
      channel: {
        ...
        session: {
          resolveStorePath: vi.fn().mockReturnValue("/tmp/kolb-bot-test-session.json"),
