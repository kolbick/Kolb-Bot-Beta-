...
import type { KolbBotPluginApi } from "kolb-bot/plugin-sdk/device-pair";
import {
...
} from "kolb-bot/plugin-sdk/device-pair";
...
async function resolveGatewayUrl(api: KolbBotPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Kolb-Bot iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Kolb-Bot iOS app:",
            "",
            "```",
            qrAscii,
            "```",
