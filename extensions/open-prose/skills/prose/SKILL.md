...
metadata: { "kolb-bot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Kolb-Bot `sessions_spawn`
- **File I/O** == Kolb-Bot `read`/`write`
- **Remote fetch** == Kolb-Bot `web_fetch` (or `exec` with curl when POST is required)
