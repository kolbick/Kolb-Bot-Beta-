---
description: Update Kolb-Bot from upstream when branch has diverged (ahead/behind)
---

# Kolb-Bot Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Kolb-Bot" || true

# Move old version
mv /Applications/Kolb-Bot.app /tmp/Kolb-Bot-backup.app

# Install new build
cp -R dist/Kolb-Bot.app /Applications/

# Launch
open /Applications/Kolb-Bot.app
```
...
pnpm kolb-bot health

# Verify no zombie processes
ps aux | grep -E "(kolb-bot|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm kolb-bot agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
morph-mcp_warpgrep_codebase_search search_string="Find deprecated FileManager.default and Thread.isMainThread usages in Swift files" repo_path="/Volumes/Main SSD/Developer/kolbis"
morph-mcp_warpgrep_codebase_search search_string="Locate Peekaboo submodule and macOS app Swift files with concurrency issues" repo_path="/Volumes/Main SSD/Developer/kolbis"
```
...
cd /Volumes/Main SSD/Developer/kolbis
...
# Update kolb-bot.json with fallback chains
...
morph-mcp_warpgrep_codebase_search search_string="Find all Swift files using deprecated FileManager.default or Thread.isMainThread" repo_path="/Volumes/Main SSD/Developer/kolbis"
```
...
pnpm kolb-bot agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
