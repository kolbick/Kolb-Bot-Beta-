---
summary: "Optional Docker-based setup and onboarding for Kolb-Bot"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Kolb-Bot on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full Kolb-Bot in Docker)
...
- builds the gateway image locally (or pulls a remote image if `KOLB_BOT_IMAGE` is set)
...
- `KOLB_BOT_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/kolbick/Kolb-Bot-Beta-:latest`)
- `KOLB_BOT_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `KOLB_BOT_EXTENSIONS` — pre-install extension dependencies at build time (space-separated extension names, e.g. `diagnostics-otel matrix`)
- `KOLB_BOT_EXTRA_MOUNTS` — add extra host bind mounts
- `KOLB_BOT_HOME_VOLUME` — persist `/home/node` in a named volume
- `KOLB_BOT_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `KOLB_BOT_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `KOLB_BOT_SANDBOX=1` for local builds.
- `KOLB_BOT_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `KOLB_BOT_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `KOLB_BOT_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `KOLB_BOT_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `KOLB_BOT_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm kolb-bot-cli dashboard --no-open`.
...
```bash
export KOLB_BOT_SANDBOX=1
export KOLB_BOT_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `kolb-bot-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `KOLB_BOT_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm kolb-bot-cli gateway probe
docker compose run -T --rm kolb-bot-cli devices list --json
```
...
`kolb-bot-cli` uses `network_mode: "service:kolb-bot-gateway"` so CLI commands can
...
network path instead of the bundled `kolb-bot-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `kolb-bot-cli`.
...
- [GitHub Container Registry package](https://github.com/kolbick/Kolb-Bot-Beta-/pkgs/container/kolb-bot)

Use image name `ghcr.io/kolbick/Kolb-Bot-Beta-` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/kolbick/Kolb-Bot-Beta-`
- `org.opencontainers.image.url=https://github.com/kolbick/Kolb-Bot-Beta-`
- `org.opencontainers.image.documentation=https://docs.github.com/kolbick/Kolb-Bot-Beta-/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=Kolb-Bot`
- `org.opencontainers.image.description=Kolb-Bot gateway and CLI runtime container image`
...
image instead, set `KOLB_BOT_IMAGE` before running the script:

```bash
export KOLB_BOT_IMAGE="ghcr.io/kolbick/Kolb-Bot-Beta-:latest"
./docker-setup.sh
```

The script detects that `KOLB_BOT_IMAGE` is not the default `kolb-bot:local` and
...
`docker-compose.yml` and helper files. `KOLB_BOT_IMAGE` skips local image build
...
Then use `kolb-dock-start`, `kolb-dock-stop`, `kolb-dock-dashboard`, etc. Run `kolb-dock-help` for all commands.

See [`ClawDock` Helper README](https://github.com/kolbick/Kolb-Bot-Beta-/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t kolb-bot:local -f Dockerfile .
docker compose run --rm kolb-bot-cli onboard
docker compose up -d kolb-bot-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`KOLB_BOT_EXTRA_MOUNTS` or `KOLB_BOT_HOME_VOLUME`, the setup script writes
...
```bash
docker compose run --rm kolb-bot-cli dashboard --no-open
docker compose run --rm kolb-bot-cli devices list
docker compose run --rm kolb-bot-cli devices approve <requestId>
```
...
`KOLB_BOT_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`kolb-bot-gateway` and `kolb-bot-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export KOLB_BOT_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `KOLB_BOT_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `KOLB_BOT_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
- If you change `KOLB_BOT_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `KOLB_BOT_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export KOLB_BOT_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `KOLB_BOT_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
dependencies into the image instead, set `KOLB_BOT_EXTENSIONS` before
running `docker-setup.sh`:

```bash
export KOLB_BOT_EXTENSIONS="diagnostics-otel matrix"
./docker-setup.sh
```
...
```bash
docker build --build-arg KOLB_BOT_EXTENSIONS="diagnostics-otel matrix" .
```
...
- If you change `KOLB_BOT_EXTENSIONS`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export KOLB_BOT_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`KOLB_BOT_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `KOLB_BOT_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `KOLB_BOT_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.kolb-bot`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/kolb-bot-config /path/to/kolb-bot-workspace
```
...
```bash
docker compose run --rm kolb-bot-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm kolb-bot-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if Kolb-Bot is still
...
```bash
docker compose exec kolb-bot-gateway node dist/index.js health --token "$KOLB_BOT_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `KOLB_BOT_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm kolb-bot-cli config set gateway.mode local
docker compose run --rm kolb-bot-cli config set gateway.bind lan
docker compose run --rm kolb-bot-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`KOLB_BOT_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.kolb-bot/agents/<agentId>/sessions/`).

### Storage model

- **Persistent host data:** Docker Compose bind-mounts `KOLB_BOT_CONFIG_DIR` to `/home/node/.kolb-bot` and `KOLB_BOT_WORKSPACE_DIR` to `/home/node/.kolb-bot/workspace`, so those paths survive container replacement.
...
- **Disk growth hotspots:** watch `media/`, `agents/<agentId>/sessions/sessions.json`, transcript JSONL files, `cron/runs/*.jsonl`, and rolling file logs under `/tmp/kolb-bot/` (or your configured `logging.file`). If you also run the macOS app outside Docker, its service logs are separate again: `~/.kolb-bot/logs/gateway.log`, `~/.kolb-bot/logs/gateway.err.log`, and `/tmp/kolbick/Kolb-Bot-Beta--gateway.log`.
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.kolb-bot/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  Kolb-Bot auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `kolb-bot sandbox recreate ...` command.
...
This builds `kolb-bot-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `kolb-bot-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "kolb-bot-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `kolb-bot-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`kolb-bot-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; Kolb-Bot provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `KOLB_BOT_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `KOLB_BOT_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `KOLB_BOT_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-kolb-bot-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-kolb-bot-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-kolb-bot-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/kolbick/Kolb-Bot-Beta-/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: Kolb-Bot runs commands with `sh -lc` (login shell), which
