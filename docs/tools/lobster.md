---
title: Lobster
summary: "Typed workflow runtime for Kolb-Bot with resumable approval gates."
description: Typed workflow runtime for Kolb-Bot — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Kolb-Bot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Kolb-Bot runs one Lobster tool call and gets a structured result.
...
Kolb-Bot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | kolb-bot.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
kolb-bot.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Kolb-Bot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Kolb-Bot Gateway (see the [Lobster repo](https://github.com/kolb-bot/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Kolb-Bot keeps core tools enabled. To restrict core
...
→ kolb-bot calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ kolb-bot calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Kolb-Bot tools that do.
