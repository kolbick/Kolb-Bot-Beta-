...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `kolb-bot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `kolb-bot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=kolb-bot-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${KOLB_BOT_STATE_DIR:-$HOME/.kolb-bot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
