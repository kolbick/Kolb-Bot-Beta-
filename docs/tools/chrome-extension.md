---
summary: "Chrome extension: let Kolb-Bot drive your existing Chrome tab"
read_when:
...
The Kolb-Bot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate kolb-bot-managed Chrome profile.
...
Kolb-Bot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Kolb-Bot release (npm package) as static files. There is no separate “build” step.

After upgrading Kolb-Bot:

- Re-run `kolb-bot browser extension install` to refresh the installed files under your Kolb-Bot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Kolb-Bot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `KOLB_BOT_GATEWAY_TOKEN`)
...
- Open the tab you want Kolb-Bot to control.
...
- `ON`: attached; Kolb-Bot can drive that tab.
...
Debugging: `kolb-bot sandbox explain`
...
`kolb-bot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `kolb-bot browser extension install` first to copy the extension to a stable location under your Kolb-Bot state directory.
...
- **This is not isolated** like the dedicated kolb-bot-managed profile.
