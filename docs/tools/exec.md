...
- `gateway`/`node` approvals are controlled by `~/.kolb-bot/exec-approvals.json`.
...
- Kolb-Bot sets `KOLB_BOT_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Kolb-Bot prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`kolb-bot security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `kolb-bot doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
