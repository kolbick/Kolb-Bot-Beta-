---
summary: "Kolb-Bot plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends Kolb-Bot with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core Kolb-Bot yet (or you want to keep optional features out of your main
install).
...
```bash
kolb-bot plugins install @kolb-bot/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@kolb-bot/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@kolb-bot/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@kolb-bot/zalouser`
- [Matrix](/channels/matrix) — `@kolb-bot/matrix`
- [Nostr](/channels/nostr) — `@kolb-bot/nostr`
- [Zalo](/channels/zalo) — `@kolb-bot/zalo`
- [Microsoft Teams](/channels/msteams) — `@kolb-bot/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

Kolb-Bot plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `kolb-bot/plugin-sdk` import when
authoring plugins:

- `kolb-bot/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `kolb-bot/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `kolb-bot/plugin-sdk/telegram` for Telegram channel plugins.
- `kolb-bot/plugin-sdk/discord` for Discord channel plugins.
- `kolb-bot/plugin-sdk/slack` for Slack channel plugins.
- `kolb-bot/plugin-sdk/signal` for Signal channel plugins.
- `kolb-bot/plugin-sdk/imessage` for iMessage channel plugins.
- `kolb-bot/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `kolb-bot/plugin-sdk/line` for LINE channel plugins.
- `kolb-bot/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `kolb-bot/plugin-sdk/acpx`, `kolb-bot/plugin-sdk/bluebubbles`,
  `kolb-bot/plugin-sdk/copilot-proxy`, `kolb-bot/plugin-sdk/device-pair`,
  `kolb-bot/plugin-sdk/diagnostics-otel`, `kolb-bot/plugin-sdk/diffs`,
  `kolb-bot/plugin-sdk/feishu`,
  `kolb-bot/plugin-sdk/google-gemini-cli-auth`, `kolb-bot/plugin-sdk/googlechat`,
  `kolb-bot/plugin-sdk/irc`, `kolb-bot/plugin-sdk/llm-task`,
  `kolb-bot/plugin-sdk/lobster`, `kolb-bot/plugin-sdk/matrix`,
  `kolb-bot/plugin-sdk/mattermost`, `kolb-bot/plugin-sdk/memory-core`,
  `kolb-bot/plugin-sdk/memory-lancedb`,
  `kolb-bot/plugin-sdk/minimax-portal-auth`,
  `kolb-bot/plugin-sdk/nextcloud-talk`, `kolb-bot/plugin-sdk/nostr`,
  `kolb-bot/plugin-sdk/open-prose`, `kolb-bot/plugin-sdk/phone-control`,
  `kolb-bot/plugin-sdk/qwen-portal-auth`, `kolb-bot/plugin-sdk/synology-chat`,
  `kolb-bot/plugin-sdk/talk-voice`, `kolb-bot/plugin-sdk/test-utils`,
  `kolb-bot/plugin-sdk/thread-ownership`, `kolb-bot/plugin-sdk/tlon`,
  `kolb-bot/plugin-sdk/twitch`, `kolb-bot/plugin-sdk/voice-call`,
  `kolb-bot/plugin-sdk/zalo`, and `kolb-bot/plugin-sdk/zalouser`.

Compatibility note:

- `kolb-bot/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `kolb-bot status`, `kolb-bot status --all`,
  `kolb-bot channels status`, `kolb-bot channels resolve`, and doctor/config
...
- Set `KOLB_BOT_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `KOLB_BOT_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `KOLB_BOT_PLUGIN_DISCOVERY_CACHE_MS` and
  `KOLB_BOT_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

Kolb-Bot scans, in order:
...
- `<workspace>/.kolb-bot/extensions/*.ts`
- `<workspace>/.kolb-bot/extensions/*/index.ts`

3. Global extensions

- `~/.kolb-bot/extensions/*.ts`
- `~/.kolb-bot/extensions/*/index.ts`

4. Bundled extensions (shipped with Kolb-Bot, mostly disabled by default)

- `<kolb-bot>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `kolb-bot plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, Kolb-Bot logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. Kolb-Bot blocks candidates when:
...
Each plugin must include a `kolb-bot.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `kolb-bot.extensions`:
...
Security guardrail: every `kolb-bot.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `kolb-bot plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `kolb-bot.channel` and
install hints via `kolb-bot.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@kolb-bot/nextcloud-talk",
  "kolb-bot": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@kolb-bot/nextcloud-talk",
...
Kolb-Bot can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.kolb-bot/mpm/plugins.json`
- `~/.kolb-bot/mpm/catalog.json`
- `~/.kolb-bot/plugins/catalog.json`

Or point `KOLB_BOT_PLUGIN_CATALOG_PATHS` (or `KOLB_BOT_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "kolb-bot": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, Kolb-Bot uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `kolb-bot.plugin.json` (`configSchema`).
...
Kolb-Bot augments `uiHints` at runtime based on discovered plugins:
...
kolb-bot plugins install <path>                 # copy a local file/dir into ~/.kolb-bot/extensions/<id>
kolb-bot plugins install ./extensions/voice-call # relative path ok
kolb-bot plugins install ./plugin.tgz           # install from a local tarball
kolb-bot plugins install ./plugin.zip           # install from a local zip
kolb-bot plugins install -l ./extensions/voice-call # link (no copy) for dev
kolb-bot plugins install @kolb-bot/voice-call # install from npm
kolb-bot plugins install @kolb-bot/voice-call --pin # store exact resolved name@version
...
If stored integrity metadata changes between updates, Kolb-Bot warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `kolb-bot voicecall`).
...
- Plugin-managed hooks show up in `kolb-bot hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `kolb-bot hooks`; enable/disable the plugin instead.
...
- When disabled, Kolb-Bot blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside Kolb-Bot (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `kolb-bot models auth login --provider <id> [--method <id>]`
...
- `config`: The current Kolb-Bot config
...
- Main package: `kolb-bot` (this repo)
- Plugins: separate npm packages under `@kolb-bot/*` (example: `@kolb-bot/voice-call`)

Publishing contract:

- Plugin `package.json` must include `kolb-bot.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `kolb-bot plugins install <npm-spec>` uses `npm pack`, extracts into `~/.kolb-bot/extensions/<id>/`, and enables it in config.
...
- CLI: `kolb-bot voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `kolb-bot.extensions` points at the built entrypoint (`dist/index.js`).
