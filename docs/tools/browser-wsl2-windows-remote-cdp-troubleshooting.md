...
read_when:
  - Running Kolb-Bot Gateway in WSL2 while Chrome lives on Windows
...
- Kolb-Bot Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/kolbick/Kolb-Bot-Beta-/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the Kolb-Bot Chrome extension.
...
- Kolb-Bot points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Kolb-Bot is not the problem yet.
...
Fix that before touching Kolb-Bot config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Kolb-Bot at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Kolb-Bot to succeed
...
```bash
kolb-bot browser open https://example.com --browser-profile remote
kolb-bot browser tabs --browser-profile remote
```

For the extension relay:

```bash
kolb-bot browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `kolb-bot browser tabs` returns the target
...
3. Kolb-Bot config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Kolb-Bot config or Control UI auth
