...
Kolb-Bot’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`kolb-bot agent`), so routing/permissions/config match your Gateway.
...
- There is no separate non-owner/per-user tool boundary on this endpoint; once a caller passes Gateway auth here, Kolb-Bot treats that caller as a trusted operator for this gateway.
...
Or target a specific Kolb-Bot agent by header:
