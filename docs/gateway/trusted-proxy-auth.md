...
read_when:
  - Running Kolb-Bot behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Kolb-Bot
...
- You run Kolb-Bot behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Kolb-Bot checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Kolb-Bot extracts the user identity from the configured header
...
- Kolb-Bot can stay on loopback HTTP behind the proxy.
...
If Kolb-Bot itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://kolb-bot.example.com
    to: http://kolb-bot-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
kolb-bot.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy kolb-bot:18789 {
...
location / {
    ...
    proxy_pass http://kolb-bot:18789;
...
`kolb-bot security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Kolb-Bot config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `kolb-bot security audit` and review findings
