...
description: "Complete field-by-field reference for ~/.kolbick/Kolb-Bot-Beta-.json"
summary: "Complete reference for every Kolb-Bot config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.kolbick/Kolb-Bot-Beta-.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Kolb-Bot uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.kolb-bot/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `kolb-bot doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `kolb-bot doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["kolb-bot-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-kolb-bot",
          requireMention: false,
...
- Kolb-Bot additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `kolb-bot plugins install @kolb-bot/mattermost`.
...
- `commands.callbackUrl` must resolve to the Kolb-Bot gateway endpoint and be reachable from the Mattermost server.
...
Kolb-Bot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `kolb-bot channels add` (or channel onboarding) while still on a single-account top-level channel config, Kolb-Bot moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `kolb-bot doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@kolb-bot", "kolb-bot"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@kolb-bot"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `kolb-bot.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Kolb-Bot auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/kolb-bot" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Kolb-Bot assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.kolb-bot/sandboxes",
        docker: {
          ...
          apparmorProfile: "kolb-bot-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "kolb-bot-sandbox-browser:bookworm-slim",
          network: "kolb-bot-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.kolb-bot/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `kolb-bot.json`.
noVNC observer access uses VNC auth by default and Kolb-Bot emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `kolb-bot-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from KOLB_BOT_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `KOLB_BOT_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `KOLB_BOT_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `KOLB_BOT_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.kolb-bot/workspace",
        agentDir: "~/.kolb-bot/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.kolb-bot/workspace-home" },
      { id: "work", workspace: "~/.kolb-bot/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Kolb-Bot resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.kolb-bot/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.kolb-bot/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.kolb-bot/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.kolb-bot/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Kolb-Bot starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.kolb-bot/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Kolb-Bot treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:kolb-bot`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.kolb-bot/attachments/<uuid>/` with a `.manifest.json`.
...
Kolb-Bot uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.kolb-bot/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `KOLB_BOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Kolb-Bot forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `kolb-bot onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `kolb-bot onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `kolb-bot onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `kolb-bot onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `kolb-bot onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `kolb-bot onboard --auth-choice minimax-api`.
...
- Loaded from `~/.kolb-bot/extensions`, `<workspace>/.kolb-bot/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `kolb-bot plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or KOLB_BOT_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `KOLB_BOT_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `KOLB_BOT_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
```bash
KOLB_BOT_CONFIG_PATH=~/.kolb-bot/a.json \
KOLB_BOT_STATE_DIR=~/.kolb-bot-a \
kolb-bot gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.kolb-bot-dev` + port `19001`), `--profile <name>` (uses `~/.kolb-bot-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.kolb-bot/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-kolb-bot-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `KOLB_BOT_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.kolb-bot/workspace/canvas",
    liveReload: true,
    // enabled: false, // or KOLB_BOT_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__kolb-bot__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__kolb-bot__/a2ui/`
...
- Also serves A2UI at `/__kolb-bot__/a2ui/`.
...
- Hostname defaults to `kolb-bot`. Override with `KOLB_BOT_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.kolb-bot/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `kolb-bot dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.kolb-bot/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${KOLB_BOT_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `kolb-bot.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      vault: {
        source: "exec",
        command: "/usr/local/bin/kolb-bot-vault-resolver",
...
- Legacy OAuth imports from `~/.kolb-bot/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/kolbick/Kolb-Bot-Beta-.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/kolbick/Kolb-Bot-Beta--YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Kolb-Bot.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `KOLB_BOT_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `kolb-bot doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `kolb-bot.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
