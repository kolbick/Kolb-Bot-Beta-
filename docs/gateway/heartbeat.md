...
- During heartbeat runs, Kolb-Bot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `kolb-bot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Kolb-Bot skips the heartbeat run entirely (no model call).
...
```bash
kolb-bot system event --text "Check for urgent follow-ups" --mode now
```
