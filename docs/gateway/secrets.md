...
Kolb-Bot supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `KOLB_BOT_GATEWAY_TOKEN` (or `KOLB_BOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Kolb-Bot runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Kolb-Bot validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Kolb-Bot QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/kolb-bot"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `kolb-bot.json` refs)
...
When reload-time activation fails after a healthy state, Kolb-Bot enters degraded secrets state.
...
- Strict command paths (for example `kolb-bot memory` remote-memory paths and `kolb-bot qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `kolb-bot status`, `kolb-bot status --all`, `kolb-bot channels status`, `kolb-bot channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `kolb-bot secrets reload`.
...
```bash
kolb-bot secrets audit --check
kolb-bot secrets configure
kolb-bot secrets audit --check
```
...
- plaintext values at rest (`kolb-bot.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `kolb-bot.json` refs)
...
- lets you select supported secret-bearing fields in `kolb-bot.json` plus `auth-profiles.json` for one agent scope
...
- `kolb-bot secrets configure --providers-only`
- `kolb-bot secrets configure --skip-provider-setup`
- `kolb-bot secrets configure --agent <id>`
...
```bash
kolb-bot secrets apply --from /tmp/kolb-bot-secrets-plan.json
kolb-bot secrets apply --from /tmp/kolb-bot-secrets-plan.json --dry-run
```
...
Kolb-Bot intentionally does not write rollback backups containing historical plaintext secret values.
