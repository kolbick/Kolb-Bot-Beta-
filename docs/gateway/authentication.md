...
Kolb-Bot supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `kolb-bot gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.kolb-bot/.env` so the daemon can read it:

```bash
cat >> ~/.kolb-bot/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `kolb-bot onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.kolb-bot/.env`, systemd/launchd).
...
```bash
kolb-bot models auth paste-token --provider anthropic
kolb-bot models auth paste-token --provider openrouter
```
...
- Priority order:
  - `KOLB_BOT_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Kolb-Bot retries with the next key only for rate-limit errors (for example
...
Run `kolb-bot models status` to confirm which profile is expiring. If the profile
