...
- `kolb-bot gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `kolb-bot doctor` reports no blocking config/service issues.
- `kolb-bot channels status --probe` shows connected/ready channels.
...
kolb-bot config get agents.defaults.models
```
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `kolb-bot configure`). If you are running Kolb-Bot via Podman using the dedicated `kolb-bot` user, the config lives at `~kolb-bot/.kolbick/Kolb-Bot-Beta-.json`.
...
kolb-bot system heartbeat last
kolb-bot logs --follow
```
...
kolb-bot approvals get --node <idOrNameOrIp>
kolb-bot logs --follow
kolb-bot status
```
...
kolb-bot browser start --browser-profile kolb-bot
...
```bash
kolb-bot gateway status
kolb-bot config get gateway.mode
kolb-bot config get gateway.remote.url
kolb-bot config get gateway.auth.mode
```
...
```bash
kolb-bot config get gateway.bind
