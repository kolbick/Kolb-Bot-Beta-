---
title: "Memory"
summary: "How Kolb-Bot memory works (workspace files + automatic memory flush)"
read_when:
...
Kolb-Bot memory is **plain Markdown in the agent workspace**. The files are the
...
`~/.kolb-bot/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

Kolb-Bot exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, Kolb-Bot triggers a **silent,
...
Kolb-Bot can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, Kolb-Bot auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. Kolb-Bot
...
BM25 + vectors + reranking. Markdown stays the source of truth; Kolb-Bot shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.kolb-bot/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.kolb-bot/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, Kolb-Bot retries with `qmd query`. If QMD fails or the binary is
  missing, Kolb-Bot automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- Kolb-Bot does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - Kolb-Bot sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index Kolb-Bot
    uses), run a one-off query with the agent’s XDG dirs.

    Kolb-Bot’s QMD state lives under your **state dir** (defaults to `~/.kolb-bot`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    Kolb-Bot uses:

    ```bash
    # Pick the same state dir Kolb-Bot uses
    STATE_DIR="${KOLB_BOT_STATE_DIR:-$HOME/.kolb-bot}"
...
- When `scope` denies a search, Kolb-Bot logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, Kolb-Bot exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.kolb-bot/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- Index storage: per-agent SQLite at `~/.kolb-bot/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, Kolb-Bot automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, Kolb-Bot combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Kolb-Bot falls back to vector-only search.
...
Kolb-Bot can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.kolb-bot/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, Kolb-Bot stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Kolb-Bot logs the
