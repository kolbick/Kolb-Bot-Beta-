...
- `/usage cost` in chats: local cost summary aggregated from Kolb-Bot session logs.
- CLI: `kolb-bot status --usage` prints a full per-provider breakdown.
- CLI: `kolb-bot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
