...
```bash
kolb-bot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `kolb-bot tui --deliver`
...
- Local shell commands receive `KOLB_BOT_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `KOLB_BOT_THEME=light` before launching `kolb-bot tui`.
- To force the original dark palette instead, set `KOLB_BOT_THEME=dark`.
...
- Check the Gateway logs: `kolb-bot logs --follow`.
- Confirm the agent can run: `kolb-bot status` and `kolb-bot models status`.
...
- No agents in picker: check `kolb-bot agents list` and your routing config.
