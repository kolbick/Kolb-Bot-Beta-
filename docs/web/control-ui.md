...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/kolb-bot`)
...
If the page fails to load, start the Gateway first: `kolb-bot gateway`.
...
you revoke it with `kolb-bot devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.kolbick/Kolb-Bot-Beta-.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop kolb-bot`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Kolb-Bot
...
```bash
kolb-bot gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Kolb-Bot **blocks** Control UI connections without device identity.
...
```bash
KOLB_BOT_CONTROL_UI_BASE_PATH=/kolb-bot/ pnpm ui:build
```
