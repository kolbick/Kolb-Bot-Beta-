---
summary: "VPS hosting hub for Kolb-Bot (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/kolb-bot-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `KOLB_BOT_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for stable startup path:
  - `KOLB_BOT_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/kolb-bot-compile-cache`
