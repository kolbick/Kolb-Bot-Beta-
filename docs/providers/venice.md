---
summary: "Use Venice AI privacy-focused models in Kolb-Bot"
read_when:
  - You want privacy-focused inference in Kolb-Bot
...
```bash
kolb-bot onboard --auth-choice venice-api-key
```
...
```bash
kolb-bot agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Kolb-Bot shows all available Venice models. Pick based on your needs:
...
```bash
kolb-bot models set venice/kimi-k2-5
kolb-bot models set venice/claude-opus-4-6
```

List all available models:

```bash
kolb-bot models list | grep venice
```

## Configure via `kolb-bot configure`

1. Run `kolb-bot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Kolb-Bot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
kolb-bot agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
kolb-bot agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
kolb-bot agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
kolb-bot agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
kolb-bot agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `kolb-bot models list` to see currently available models. Some models may be temporarily offline.
