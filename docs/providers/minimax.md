---
summary: "Use MiniMax M2.5 in Kolb-Bot"
read_when:
  - You want MiniMax models in Kolb-Bot
...
```bash
kolb-bot plugins enable minimax-portal-auth  # skip if already loaded.
kolb-bot gateway restart  # restart if gateway is already running
kolb-bot onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/kolbick/Kolb-Bot-Beta-/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `kolb-bot.json`:
...
- Use `kolb-bot models list` and `kolb-bot models set minimax/MiniMax-M2.5` to switch.
...
- Running `kolb-bot configure` and selecting **MiniMax M2.5**, or
