---
summary: "Use Synthetic's Anthropic-compatible API in Kolb-Bot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Kolb-Bot registers it as the
...
```bash
kolb-bot onboard --auth-choice synthetic-api-key
```
...
Note: Kolb-Bot's Anthropic client appends `/v1` to the base URL, so use
