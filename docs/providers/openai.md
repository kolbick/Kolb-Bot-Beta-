---
summary: "Use OpenAI via API keys or Codex subscription in Kolb-Bot"
read_when:
  - You want to use OpenAI models in Kolb-Bot
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Kolb-Bot.
...
```bash
kolb-bot onboard --auth-choice openai-api-key
# or non-interactive
kolb-bot onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Kolb-Bot forwards both through the `openai/*` Responses path.
...
kolb-bot onboard --auth-choice openai-codex

# Or run OAuth directly
kolb-bot models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Kolb-Bot
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

Kolb-Bot uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Kolb-Bot also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Kolb-Bot enables it by default for
...
Kolb-Bot, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), Kolb-Bot now auto-enables OpenAI server-side
