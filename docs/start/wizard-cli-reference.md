...
read_when:
  - You need detailed behavior for kolb-bot onboard
...
This page is the full reference for `kolb-bot onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.kolbick/Kolb-Bot-Beta-.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `kolb-bot doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `kolb-bot pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `kolb-bot health`.
    - `kolb-bot status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.kolb-bot/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.kolb-bot/agents/<agentId>/agent/auth-profiles.json`
...
`~/.kolb-bot/credentials/oauth.json` (or `$KOLB_BOT_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`kolb-bot onboard`](/cli/onboard)
