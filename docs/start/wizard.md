...
The onboarding wizard is the **recommended** way to set up Kolb-Bot on macOS,
...
`kolb-bot dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`kolb-bot configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.kolb-bot/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `kolb-bot doctor` first.
</Note>
...
Use `kolb-bot agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.kolb-bot/workspace-<agentId>`.
...
- CLI command reference: [`kolb-bot onboard`](/cli/onboard)
