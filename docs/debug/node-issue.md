...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Kolb-Bot
...
Running Kolb-Bot via Node with `tsx` fails at startup with:

```
[kolb-bot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `kolb-bot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch kolb-bot.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node kolb-bot.mjs status` works on Node 25.
