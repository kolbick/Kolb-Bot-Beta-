---
summary: "CLI reference for `kolb-bot plugins` (list, install, uninstall, enable/disable, doctor)"
read_when:
...
kolb-bot plugins uninstall <id>
kolb-bot plugins doctor
kolb-bot plugins update <id>
kolb-bot plugins update --all
```

Bundled plugins ship with Kolb-Bot but start disabled. Use `plugins enable` to
activate them.
...
kolb-bot plugins install <npm-spec> --pin
```
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Kolb-Bot
...
```bash
kolb-bot plugins uninstall <id>
kolb-bot plugins uninstall <id> --dry-run
kolb-bot plugins uninstall <id> --keep-files
```
...
state dir extensions root (`$KOLB_BOT_STATE_DIR/extensions/<id>`). Use
