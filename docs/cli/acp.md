...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a Kolb-Bot Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`kolb-bot acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Kolb-Bot gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
kolb-bot acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
kolb-bot acp --url wss://gateway-host:18789 --token-file ~/.kolb-bot/gateway.token

# Attach to an existing session key
kolb-bot acp --session agent:main:main

# Attach by label (must already exist)
kolb-bot acp --session-label "support inbox"

# Reset the session key before the first prompt
kolb-bot acp --session agent:main:main --reset-session
```
...
kolb-bot acp client --server-args --url wss://gateway-host:18789 --token-file ~/.kolb-bot/gateway.token

# Override the server command (default: kolb-bot)
kolb-bot acp client --server "node" --server-args kolb-bot.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a Kolb-Bot Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `kolb-bot acp` over stdio.

Example config (persisted):

```bash
kolb-bot config set gateway.remote.url wss://gateway-host:18789
kolb-bot config set gateway.remote.token <token>
```
...
```bash
kolb-bot acp --session agent:main:main
kolb-bot acp --session agent:design:main
kolb-bot acp --session agent:qa:bug-123
```
...
Kolb-Bot bot over ACP, use `acpx` with its built-in `kolb-bot` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx kolb-bot` at `kolb-bot acp`.
3. Target the Kolb-Bot session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Kolb-Bot ACP session
acpx kolb-bot exec "Summarize the active Kolb-Bot session state."

# Persistent named session for follow-up turns
acpx kolb-bot sessions ensure --name codex-bridge
acpx kolb-bot -s codex-bridge --cwd /path/to/repo \
  "Ask my Kolb-Bot work agent for recent context relevant to this repo."
```

If you want `acpx kolb-bot` to target a specific Gateway and session key every
time, override the `kolb-bot` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "kolb-bot": {
      "command": "env KOLB_BOT_HIDE_BANNER=1 KOLB_BOT_SUPPRESS_NOTES=1 kolb-bot acp --url ws://127.0.0.1:18789 --token-file ~/.kolb-bot/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Kolb-Bot checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env KOLB_BOT_HIDE_BANNER=1 KOLB_BOT_SUPPRESS_NOTES=1 node kolb-bot.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Kolb-Bot agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Kolb-Bot ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`KOLB_BOT_GATEWAY_TOKEN`, `KOLB_BOT_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`KOLB_BOT_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback when `gateway.auth.*` is unset
...
- ACP runtime backend child processes receive `KOLB_BOT_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `kolb-bot acp client` sets `KOLB_BOT_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `kolb-bot`).
