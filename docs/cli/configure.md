---
summary: "CLI reference for `kolb-bot configure` (interactive configuration prompts)"
read_when:
...
Tip: `kolb-bot config` without a subcommand opens the same wizard. Use
`kolb-bot config get|set|unset` for non-interactive edits.
...
```bash
kolb-bot configure
kolb-bot configure --section model --section channels
```
