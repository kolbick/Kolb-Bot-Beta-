...
- Publish as `kolb-bot/plugin-sdk` (or export from core under `kolb-bot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `Kolb-BotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: KolbBotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: KolbBotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: KolbBotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: KolbBotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: KolbBotConfig): string;
  };
};
```
...
- Introduce `kolb-bot/plugin-sdk`.
- Add `api.runtime` to `Kolb-BotPluginApi` with the surface above.
