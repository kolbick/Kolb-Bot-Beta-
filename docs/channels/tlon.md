...
Tlon is a decentralized messenger built on Urbit. Kolb-Bot connects to your Urbit ship and can
...
```bash
kolb-bot plugins install @kolb-bot/tlon
```

Local checkout (when running from a git repo):

```bash
kolb-bot plugins install ./extensions/tlon
```
...
By default, Kolb-Bot blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `kolb-bot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Kolb-Bot replies in-thread.
