...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Kolb-Bot to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Kolb-Bot releases, so no separate plugin install
is required.
...
```bash
kolb-bot plugins install @kolb-bot/feishu
```
...
If you just installed Kolb-Bot, run the wizard:
...
2. Configuring app credentials in Kolb-Bot
...
1. You already ran `kolb-bot channels add` for Feishu
2. The gateway is running (`kolb-bot gateway status`)
...
Edit `~/.kolbick/Kolb-Bot-Beta-.json`:
...
```bash
kolb-bot pairing approve feishu <CODE>
```
...
2. Run `kolb-bot logs --follow` and look for `chat_id`
...
2. Run `kolb-bot logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `kolb-bot gateway status`  | Show gateway status           |
| `kolb-bot gateway install` | Install/start gateway service |
| `kolb-bot gateway stop`    | Stop gateway service          |
| `kolb-bot gateway restart` | Restart gateway service       |
| `kolb-bot logs --follow`   | Tail gateway logs             |
...
4. Check logs: `kolb-bot logs --follow`
...
5. Ensure the gateway is running: `kolb-bot gateway status`
6. Check logs: `kolb-bot logs --follow`
