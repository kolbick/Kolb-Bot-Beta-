...
Kolb-Bot routes replies **back to the channel where a message came from**. The
...
Kolb-Bot infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Kolb-Bot still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Kolb-Bot would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.kolb-bot/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.kolb-bot`):

- `~/.kolb-bot/agents/<agentId>/sessions/sessions.json`
