---
summary: "Synology Chat webhook setup and Kolb-Bot config"
read_when:
  - Setting up Synology Chat with Kolb-Bot
...
```bash
kolb-bot plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your Kolb-Bot gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in Kolb-Bot.
...
- Pairing approvals work with:
  - `kolb-bot pairing list synology-chat`
  - `kolb-bot pairing approve synology-chat <CODE>`
...
```bash
kolb-bot message send --channel synology-chat --target 123456 --text "Hello from Kolb-Bot"
kolb-bot message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
