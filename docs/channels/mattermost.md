---
summary: "Mattermost bot setup and Kolb-Bot config"
read_when:
...
```bash
kolb-bot plugins install @kolb-bot/mattermost
```

Local checkout (when running from a git repo):

```bash
kolb-bot plugins install ./extensions/mattermost
```
...
4. Configure Kolb-Bot and start the gateway.
...
Native slash commands are opt-in. When enabled, Kolb-Bot registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Kolb-Bot derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Kolb-Bot.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Kolb-Bot.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Kolb-Bot, not `404`.
...
- Approve via:
  - `kolb-bot pairing list mattermost`
  - `kolb-bot pairing approve mattermost <CODE>`
...
Use these target formats with `kolb-bot message send` or cron/webhooks:
...
- If `interactions.callbackBaseUrl` is omitted, Kolb-Bot derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Kolb-Bot run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="kolb-bot-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"kolb-bot-mattermost-interactions",
...
`kolb-bot message send` and cron/webhook deliveries.
