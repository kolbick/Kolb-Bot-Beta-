...
```bash
kolb-bot plugins install @kolb-bot/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
kolb-bot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Kolb-Bot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `kolb-bot pairing list nextcloud-talk`
  - `kolb-bot pairing approve nextcloud-talk <CODE>`
