...
- Kolb-Bot installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Kolb-Bot"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Kolb-Bot and restart the gateway.
5. Send a first DM and approve pairing (`kolb-bot pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Kolb-Bot"` then scan the QR in Signal.
...
4. Configure Kolb-Bot, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `kolb-bot pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Kolb-Bot at it:
...
This skips auto-spawn and the startup wait inside Kolb-Bot. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `kolb-bot pairing list signal`
  - `kolb-bot pairing approve signal <CODE>`
...
- **Typing indicators**: Kolb-Bot sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Kolb-Bot forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `kolb-bot doctor --fix`.
...
kolb-bot pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/kolbick/Kolb-Bot-Beta--$(date +%Y-%m-%d).log" | tail -20
```
