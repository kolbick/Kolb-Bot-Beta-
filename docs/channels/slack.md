...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Kolb-Bot HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `kolb-bot pairing approve slack <code>`.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Kolb-Bot is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Kolb-Bot",
    "description": "Slack connector for Kolb-Bot"
  },
  "features": {
    "bot_user": {
      "display_name": "Kolb-Bot",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/kolb-bot",
        "description": "Send a message to Kolb-Bot",
        "should_escape": false
      }
    ]
  },
...
Kolb-Bot supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Kolb-Bot falls back to normal delivery for remaining payloads.
