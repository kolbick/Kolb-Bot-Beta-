...
read_when:
  - Setting up Zalo Personal for Kolb-Bot
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Kolb-Bot.
...
- Install via CLI: `kolb-bot plugins install @kolb-bot/zalouser`
- Or from a source checkout: `kolb-bot plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `kolb-bot channels login --channel zalouser`
...
```bash
kolb-bot directory self --channel zalouser
kolb-bot directory peers list --channel zalouser --query "name"
kolb-bot directory groups list --channel zalouser --query "work"
```
...
- `kolb-bot pairing list zalouser`
- `kolb-bot pairing approve zalouser <code>`
...
- On startup, Kolb-Bot resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, Kolb-Bot stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Kolb-Bot state. Example:
...
- Kolb-Bot sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Kolb-Bot sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `kolb-bot channels logout --channel zalouser && kolb-bot channels login --channel zalouser`
...
- The channel now runs fully in Kolb-Bot without external CLI binaries.
