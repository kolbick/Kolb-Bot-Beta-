...
Matrix is an open, decentralized messaging protocol. Kolb-Bot connects as a Matrix **user**
...
```bash
kolb-bot plugins install @kolb-bot/matrix
```

Local checkout (when running from a git repo):

```bash
kolb-bot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `kolb-bot plugins install @kolb-bot/matrix`
   - From a local checkout: `kolb-bot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Kolb-Bot calls the same
     login endpoint, stores the access token in `~/.kolb-bot/credentials/matrix/credentials.json`,
...
- On first connection, Kolb-Bot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Kolb-Bot logs a warning.
...
`~/.kolb-bot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `kolb-bot pairing list matrix`
  - `kolb-bot pairing approve matrix <CODE>`
...
- On startup, Kolb-Bot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
