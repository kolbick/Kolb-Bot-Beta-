...
read_when:
  - Setting up Twitch chat integration for Kolb-Bot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Kolb-Bot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
kolb-bot plugins install @kolb-bot/twitch
```

Local checkout (when running from a git repo):

```bash
kolb-bot plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `KOLB_BOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "kolb-bot", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use KOLB_BOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
KOLB_BOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
