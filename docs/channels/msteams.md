...
```bash
kolb-bot plugins install @kolb-bot/msteams
```

Local checkout (when running from a git repo):

```bash
kolb-bot plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Kolb-Bot will offer the local install path automatically.
...
3. Configure Kolb-Bot with those credentials.
...
- Talk to Kolb-Bot via Teams DMs, group chats, or channels.
...
- On startup, Kolb-Bot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.kolbick/Kolb-Bot-Beta-.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Kolb-Bot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `kolb-bot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `kolb-bot plugins install @kolb-bot/msteams`
   - From a local checkout: `kolb-bot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Kolb-Bot in Teams", "full": "Kolb-Bot in Teams" },
...
Kolb-Bot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Kolb-Bot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/Kolb-BotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Kolb-Bot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `kolb-bot message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.kolb-bot/msteams-polls.json`.
...
```bash
kolb-bot message send --channel msteams \
...
kolb-bot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
kolb-bot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
kolb-bot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
kolb-bot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
