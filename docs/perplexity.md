...
Kolb-Bot supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Kolb-Bot also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `kolb-bot configure --section web`. It stores the key in
`~/.kolbick/Kolb-Bot-Beta-.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.kolb-bot/.env` (or your service environment). See [Env vars](/help/faq#how-does-kolb-bot-load-environment-variables).
