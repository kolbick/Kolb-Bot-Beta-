...
This plan defines how Kolb-Bot should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Kolb-Bot core owns ACP control plane concerns
...
Kolb-Bot should not reimplement ACP transport internals in core.
Kolb-Bot should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Kolb-Bot, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Kolb-Bot state dir
...
- ACP event stream is normalized to Kolb-Bot reply chunks
...
For the first production backend (`extensions/acpx`), Kolb-Bot and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Kolb-Bot `sessionKey`)
...
- Kolb-Bot consumes ndjson events from `acpx --format json --json-strict`
