---
summary: "Research notes: offline memory system for Kolb workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.kolb-bot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Kolb-Bot integration
...
Target: Kolb-style workspace (`agents.defaults.workspace`, default `~/.kolb-bot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.kolb-bot/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Kolb to see every session.
...
```
~/.kolb-bot/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Kolb-Bot**, but keep a separable core library.

### Why integrate into Kolb-Bot?

- Kolb-Bot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `kolb-bot memory recall "…" --k 25 --since 30d`
  - `kolb-bot memory reflect --since 7d`
...
Pragmatic take for `~/.kolb-bot/workspace`:
