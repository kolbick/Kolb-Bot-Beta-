...
Kolb-Bot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Kolb-Bot owns lifecycle when `channels.signal.autoStart=true`.
...
- Kolb-Bot spawns `imsg rpc` as a child process (legacy iMessage integration).
