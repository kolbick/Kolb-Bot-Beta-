---
summary: "How Kolb-Bot builds prompt context and reports token usage + costs"
read_when:
...
Kolb-Bot tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Kolb-Bot assembles its own system prompt on every run. It includes:
...
For images, Kolb-Bot downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Kolb-Bot session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `kolb-bot status --usage` and `kolb-bot channels list` show
...
`cacheWrite`. If pricing is missing, Kolb-Bot shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Kolb-Bot can
...
Anthropic's 1M context window is currently beta-gated. Kolb-Bot can inject the
...
Kolb-Bot skips the `context-1m-*` beta header because Anthropic currently
