...
Kolb-Bot features that can generate provider usage or paid API calls.
...
- `kolb-bot status --usage` and `kolb-bot channels list` show provider **usage windows**
...
Kolb-Bot can pick up credentials from:
...
- `kolb-bot status --usage`
- `kolb-bot models status --json`
...
`kolb-bot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
