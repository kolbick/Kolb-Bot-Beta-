...
This is the full reference for the `kolb-bot onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.kolb-bot/credentials/oauth.json`; auth profiles live in `~/.kolb-bot/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.kolb-bot/credentials/oauth.json` (or `$KOLB_BOT_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.kolb-bot/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `kolb-bot pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `kolb-bot configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `kolb-bot health`.
    - Tip: `kolb-bot status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
kolb-bot onboard --non-interactive \
  --mode local \
  --auth-choice apiKey \
...
```bash
export KOLB_BOT_GATEWAY_TOKEN="your-token"
kolb-bot onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env KOLB_BOT_GATEWAY_TOKEN
```
...
- Stores it under `~/.kolb-bot/tools/signal-cli/<version>/`.
...
Typical fields in `~/.kolbick/Kolb-Bot-Beta-.json`:
...
`kolb-bot agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.kolb-bot/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.kolb-bot/agents/<agentId>/sessions/`.
