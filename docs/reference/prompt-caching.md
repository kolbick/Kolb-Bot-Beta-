...
- With Anthropic API-key auth profiles, Kolb-Bot seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Kolb-Bot injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Kolb-Bot exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.kolb-bot/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$KOLB_BOT_STATE_DIR/logs/cache-trace.jsonl`
...
- `KOLB_BOT_CACHE_TRACE=1` enables cache tracing.
- `KOLB_BOT_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `KOLB_BOT_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `KOLB_BOT_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `KOLB_BOT_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
