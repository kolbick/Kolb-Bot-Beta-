...
This document explains how Kolb-Bot manages sessions end-to-end:
...
Kolb-Bot is designed around a single **Gateway process** that owns session state.
...
Kolb-Bot persists sessions in two layers:
...
- Transcripts: `~/.kolb-bot/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Kolb-Bot resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Kolb-Bot reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.kolb-bot/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Kolb-Bot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Kolb-Bot consumes the events, but Pi decides when to compact).
...
Kolb-Bot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Kolb-Bot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Kolb-Bot leaves it alone.
...
- `kolb-bot status` (CLI)
- `kolb-bot sessions` / `sessions --json`
...
Kolb-Bot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Kolb-Bot strips/suppresses this in the delivery layer.

As of `2026.1.10`, Kolb-Bot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Kolb-Bot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Kolb-Bot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `kolb-bot status`.
