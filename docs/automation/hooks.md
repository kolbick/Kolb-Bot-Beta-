...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in Kolb-Bot.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Kolb-Bot. See [Webhook Hooks](/automation/webhook) or use `kolb-bot webhooks` for Gmail helper commands.
...
- Extend Kolb-Bot's behavior without modifying core code

## Getting Started

### Bundled Hooks

Kolb-Bot ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.kolb-bot/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.kolb-bot/logs/commands.log`
...
During onboarding (`kolb-bot onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.kolb-bot/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<kolb-bot>/dist/hooks/bundled/` (shipped with Kolb-Bot)
...
Hook packs are standard npm packages that export one or more hooks via `kolb-bot.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.kolb-bot/hooks/<id>`.
Each `kolb-bot.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `kolb-bot hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.github.com/kolbick/Kolb-Bot-Beta-/automation/hooks#my-hook
metadata:
...
The `metadata.kolb-bot` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Kolb-Bot persists them.
...
- **Managed hooks** (`~/.kolb-bot/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.kolb-bot/hooks/my-hook
cd ~/.kolb-bot/hooks/my-hook
```
...
metadata: { "kolb-bot": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
kolb-bot hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.kolb-bot/workspace`)
...
```yaml
metadata: { "kolb-bot": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "kolb-bot": { "events": ["command"] } } # General - more overhead
```
...
```bash
# macOS
./scripts/kolblog.sh -f

# Other platforms
tail -f ~/.kolb-bot/gateway.log
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.kolb-bot/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.kolb-bot/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.kolb-bot/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/kolbick/Kolb-Bot-Beta-/tree/main/src/hooks/bundled)
