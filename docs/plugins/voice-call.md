...
read_when:
  - You want to place an outbound voice call from Kolb-Bot
...
Voice calls for Kolb-Bot via a plugin. Supports outbound notifications and
...
- Use `kolb-bot voicecall ...` or the `voice_call` tool
...
```bash
kolb-bot plugins install ./extensions/voice-call
