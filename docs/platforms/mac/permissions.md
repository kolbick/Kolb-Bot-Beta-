...
- Same path: run the app from a fixed location (for Kolb-Bot, `dist/Kolb-Bot.app`).
...
```bash
sudo tccutil reset Accessibility ai.kolb-bot.mac
sudo tccutil reset ScreenCapture ai.kolb-bot.mac
...
Workaround: move files into the Kolb-Bot workspace (`~/.kolb-bot/workspace`) if you want to avoid per-folder grants.
