...
service via the external `kolb-bot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.kolb-bot.gateway`
  (or `ai.kolb-bot.<profile>` when using `--profile`/`KOLB_BOT_PROFILE`; legacy `com.kolb-bot.*` is supported).
