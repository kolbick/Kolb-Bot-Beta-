---
summary: "Kolb-Bot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Kolb-Bot on a Raspberry Pi
  - Running Kolb-Bot on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Kolb-Bot on Raspberry Pi

## Goal

Run a persistent, always-on Kolb-Bot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
kolb-bot config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/kolb-bot-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/kolb-bot-compile-cache
mkdir -p /var/tmp/kolb-bot-compile-cache
export KOLB_BOT_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `KOLB_BOT_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Kolb-Bot, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit kolb-bot
```

```ini
[Service]
Environment=KOLB_BOT_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/kolb-bot-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Kolb-Bot state/cache on SSD-backed storage to avoid SD-card
...
Most Kolb-Bot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled kolb-bot

# Enable if not
sudo systemctl enable kolb-bot

# Start on boot
sudo systemctl start kolb-bot
```
...
```bash
# Check logs
journalctl -u kolb-bot --no-pager -n 100

# Common fix: rebuild
cd ~/kolb-bot  # if using hackable install
npm run build
sudo systemctl restart kolb-bot
```
